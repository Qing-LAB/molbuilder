...
def test_bulk_z_period_admits_what_the_REAL_path_does_to_a_built_lead():
    """The tolerance's whole job, and its only one.

    `modify.add_slab` gives layer z-values that are copies of one number
    (measured spread 8.9e-16 A).  The lead then reaches `bulk_z_period`
    through exactly one door -- a `.xyz` + `.molstruct.json` pair, which
    ...
    cellmod.bulk_z_period(exact)                       # as built

    six_dp = [round(z, 6) for z in exact]              # through the codec
    gaps = np.diff(six_dp)
    assert gaps.max() - gaps.min() <= 1e-6 + 1e-12, (
        "the codec's own quantization is the only noise on this path")
    zper, spacing, n = cellmod.bulk_z_period(six_dp)
    assert n == 6
    assert spacing == pytest.approx(d, abs=1e-6)
