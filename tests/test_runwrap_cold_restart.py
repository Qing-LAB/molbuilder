...
# --------------------------------------------------------------------- #
#  The sweep is by NAME, not by an extension list (2026-08-08)          #
# --------------------------------------------------------------------- #
#
# `job-contracts.md § 4.1`: an enumeration of what an engine writes cannot be
# complete -- SIESTA's output set depends on its version and its options -- and
# the failure is silent in the worst direction, since --cold's entire purpose
# is leaving nothing behind.  So everything under the id is swept EXCEPT the
# files molbuilder wrote, which is the list that CAN be complete.
#
# Run against the REAL generated bash rather than the Python that emits it:
# this moves files, and the only thing worth asserting is what ends up where.

import subprocess


def _run_cold_block(tmp_path, label, names):
    """Materialise *names* in a temp dir and execute the wrapper's cold block."""
    from pathlib import Path as _P

    from molbuilder.runwrap import render_run_wrapper
    text = render_run_wrapper(_P(f"/x/{label}.fdf"))
    ...
    (tmp_path / f"{label}.fdf").write_text(f"SystemLabel {label}\n")

    subprocess.run(["bash", "cold.sh"], cwd=tmp_path,
                   capture_output=True, check=False)
    aside = list(tmp_path.glob(f"{label}-restart-aside-*"))
    swept = {p.name for p in aside[0].iterdir()} if aside else set()
    kept = {p.name for p in tmp_path.iterdir()
            if p.is_file() and p.name != "cold.sh"}
    return kept, swept


_LABEL = "bdt-relax"
_OURS = [f"{_LABEL}.fdf", f"{_LABEL}.fdf.template", f"{_LABEL}_coarse.fdf",
         f"{_LABEL}.run.sh", f"{_LABEL}.sbatch", f"{_LABEL}.molwatch.log",
         f"{_LABEL}.log", f"{_LABEL}-run0.out", f"{_LABEL}-run1.out"]
_ENGINE = [f"{_LABEL}.XV", f"{_LABEL}.DM", f"{_LABEL}.CG", f"{_LABEL}.TSHS",
           f"{_LABEL}.EIG", f"{_LABEL}.STRUCT_OUT", f"{_LABEL}.ANI"]


def test_cold_keeps_every_file_molbuilder_wrote(tmp_path):
    """The half that has to be exact. A wrong keep-list moves the deck the
    run is about to use, or the run's own history -- the ``-runN.out`` files
    a user goes back to read."""
    kept, swept = _run_cold_block(tmp_path, _LABEL, _OURS + _ENGINE)
    assert set(_OURS) <= kept
    assert not (set(_OURS) & swept)


def test_cold_sweeps_engine_state_including_what_no_list_names(tmp_path):
    """**The reason for the change.** ``.SOMETHING_NEW`` appears in none of
    this project's inventories; under the old extension loop it was left
    behind, so a run asked to start clean did not."""
    novel = f"{_LABEL}.SOMETHING_NEW_IN_SIESTA_6"
    kept, swept = _run_cold_block(tmp_path, _LABEL, _OURS + _ENGINE + [novel])
    assert novel in swept
    assert set(_ENGINE) <= swept


def test_cold_does_not_reach_a_neighbours_files(tmp_path):
    """A job called ``bdt-relax`` must not sweep ``bdt-relax2``'s state.

    The globs require a **separator** after the label for exactly this: a
    bare ``"$_warm_label"*`` would match the neighbour, and one directory
    holding two jobs is a mess the user is entitled to make."""
    other = "bdt-relax2.XV"
    kept, swept = _run_cold_block(tmp_path, _LABEL, _OURS + _ENGINE + [other])
    assert other in kept
    assert other not in swept


def test_cold_moves_and_never_deletes(tmp_path):
    """§ 4.1's standing promise, and half of why sweeping wide is safe: the
    files go into ``<label>-restart-aside-<UTC>/``. The checkpoint history is
    the other half."""
    kept, swept = _run_cold_block(tmp_path, _LABEL, _OURS + _ENGINE)
    assert swept, "nothing was moved aside at all"
    aside = list(tmp_path.glob(f"{_LABEL}-restart-aside-*"))
    assert len(aside) == 1 and aside[0].is_dir()
    assert all((aside[0] / n).read_text() == "x\n" for n in _ENGINE)
