"""/api/state-timeline/* — the workspace session state-timeline (blueprints/state_timeline.py).
...
def client_project(tmp_path, monkeypatch):
    ...
    import molbuilder.web.blueprints.state_timeline as wsbp
...
def test_state_timeline_roundtrip_and_prune(client_project):
    ...
    r = _json(client.post("/api/state-timeline/prune",
...
def test_state_write_keeps_rolling_window(client_project):
    ...
    ws = "ws-window"
    for i in range(35):
        client.post("/api/state-timeline/write",
...
def test_state_rejects_bad_identity(client_project):
    ...
    client, proj = client_project
    assert client.post("/api/state-timeline/write",
