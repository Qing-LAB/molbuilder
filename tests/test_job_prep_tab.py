"""Job Prep tab — the page renders, and it renders the design's shape.

`docs/web/job-prep.md` is the contract. This pins the parts a reader of that
...
easy to lose in a later edit:
  ...
  * the tab sheet is **layer 5** of `docs/web/ui-contract.md` § 1 — composition
    ...
    `job-prep/style.css` is the drift that made the older tab sheets
...
SHEET  = STATIC / "job-prep/style.css"
VIEWER = STATIC / "job-prep/viewer.js"
...
def test_the_tab_is_in_the_roster_and_routes():
    ...
    entry = next((t for t in TABS if t["key"] == "job-prep"), None)
    assert entry is not None, "job-prep missing from TABS"
    assert entry["path"] == "/job-prep"
    assert entry["label"] == "Job prep"


def test_the_page_renders_the_designs_parts(web_client):
    body = web_client.get("/job-prep").data.decode()
    for needle in (
        'id="jp-path"',            # § 2  where it saves
        'id="jp-state"',           # § 2  the three folder states
        'id="jp-stage-table"',     # § 5  stages
        'id="jp-machine-rows"',    # § 6  the machine half
        'id="jp-file-list"',       # what will be written
        'id="jp-editor"',          # the CodeMirror mount
        "job-prep/style.css",
        "job-prep/viewer.js",
    ):
        assert needle in body, f"missing {needle!r} on /job-prep"
...
def test_the_tab_writes_nothing_yet(web_client):
    ...
    """
    body = web_client.get("/job-prep").data.decode()
    m = re.search(r"<button[^>]*id=\"jp-save\"[^>]*>", body)
...
def test_the_page_loads_the_shared_stylesheet_layers(web_client):
    ...
    order = [body.index(x) for x in (
        ...
        "lib/form-components.css", "job-prep/style.css")]
...
def test_the_sheet_writes_no_raw_palette_colour():
    ...
    assert not offenders, (
        "raw colours in job-prep/style.css — use a var(--token) from "
    ...
    for prop, val in _declarations(SHEET.read_text()):
        ...
        for tok in val.split():
            if re.fullmatch(r"-?\d*\.?\d+(px|rem|em)", tok) and not tok.startswith("0"):
                ...
                break
    assert not offenders, (
        "magic numbers in job-prep/style.css — use --space-*, --text-* or "
        f"--radius*: {offenders}")


def test_every_jp_token_the_sheet_uses_is_defined():
    """A `var(--jp-x)` with no definition renders as nothing at all."""
    tokens = (STATIC / "lib/tokens.css").read_text()
    defined = set(re.findall(r"(--jp-[a-z0-9-]+)\s*:", tokens))
    used    = set(re.findall(r"var\((--jp-[a-z0-9-]+)", SHEET.read_text()))
    assert used <= defined, f"undefined --jp-* tokens: {sorted(used - defined)}"
    assert defined, "no --jp-* tokens declared in lib/tokens.css"
...
def test_the_jp_tokens_live_in_the_one_palette_file():
    ...
    assert ":root" not in SHEET.read_text(), (
        "job-prep/style.css declares its own :root block — module tokens "
...
def test_the_editor_uses_the_shared_codemirror_loader():
    ...
    for copied in ("_injectScript", "injectScript(", "codemirror.min.js"):
        assert copied not in src, (
            f"{copied!r} in job-prep/viewer.js — the bundle's asset list has "
            "ONE home, and the vendor-integrity test reads it there")


def test_the_editor_asks_for_no_mode_the_bundle_lacks():
    """The vendored 5.65.16 bundle ships markdown and no json/javascript mode.

    Asking for one silently yields plain text, which is the same result with a
    misleading line of code.  The controller passes `mode: null` on purpose.
    """
    src = VIEWER.read_text()
    assert re.search(r"mode:\s*null", src), "the editor should ask for mode: null"
    for absent in ("application/json", '"javascript"', "'javascript'"):
        assert absent not in src, (
            f"{absent!r} is not a mode this bundle carries — vendoring one is "
