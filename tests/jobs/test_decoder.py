"""L3 tests for molbuilder.jobs.decoder.decode_run_dir.

Pins docs/protocols/job-decoder.md § 2-7 against real project
fixtures:
...
Both fixtures live in the user's project tree and are real running
data, NOT synthetic.  If they move or are deleted the tests skip
...
from molbuilder.jobs.decoder import (
    SCHEMA_VERSION,
    ENGINE_BODY_KEYS,
...
def test_decode_tj_bdt_au111_smoke():
    """decode_run_dir on TJ-BDT-Au111 returns a v1 document with all
    top-level keys per § 2."""
    decoded = decode_run_dir(_need_dir(TJ_DIR))
    for key in (
        "schema_version", "decoded_at", "decoder_version",
        "job_type", "engine", "system_label", "run_dir",
        "status", "progress", "geometry", "plots",
        "source_files", "engine_input_by_stage",
        "parse_warnings", "diagnostics",
    ):
        assert key in decoded, f"missing top-level key {key!r}"
...
def test_decode_bdt_withaujunction_smoke():
    """decode_run_dir on BDT-withAuJunction (multi-stage, no
    script-contract block) returns a v1 document with the same
    top-level shape."""
...
def test_job_type_ambiguous_raises(tmp_path):
    ...
    from molbuilder.jobs.decoder import _classify_job_type
    text = (
        "SystemLabel test\n"
        "MD.TypeOfRun CG\n"
        "MD.NumCGsteps 200\n"
...
def test_engine_input_envelope_no_script_contract_for_bdt():
    ...
    envelope still parses; the *present* flags are False; the
    engine_body_summary still extracts engine-body directives."""
    decoded = decode_run_dir(_need_dir(BDT_DIR))
    ei_map = decoded["engine_input_by_stage"]
    assert len(ei_map) >= 1
...
def test_engine_body_summary_curated_keys_are_exact():
    ...
    — no more, no less.  Catches accidental schema drift."""
    decoded = decode_run_dir(_need_dir(TJ_DIR))
    ei_map = decoded["engine_input_by_stage"]
...
def test_engine_body_summary_raw_string_values():
    ...
    ei_map = decoded["engine_input_by_stage"]
    summary = next(iter(ei_map.values()))["engine_body_summary"]
    # Spot-check a few known-present keys
    ...
    # MeshCutoff carries unit verbatim
...
def test_multistage_plot_buckets_keyed_by_out_filename():
    """BDT-withAuJunction has multiple .out files (stage1-runN +
    stage3-runM).  Each appears as its own plot bucket keyed by
    literal filename."""
    decoded = decode_run_dir(_need_dir(BDT_DIR))
    etot = decoded["plots"]["etot_per_cg"]
    # At least one .out should have produced data
    assert len(etot) >= 1, "no .out files produced etot data"
    # All bucket keys are real .out filenames in the dir
...
def test_geometry_carries_xyz_and_cell():
    ...
    assert len(geom["regions"]) >= 3      # L-electrode, R-electrode, bridge
...
def test_progress_carries_cg_step_count():
    """BDT-withAuJunction finished stage3 — current_cg_step should
    be > 0."""
    decoded = decode_run_dir(_need_dir(BDT_DIR))
...
def test_no_direct_out_grep_in_decoder():
    """The decoder module must NOT contain regex/grep calls against
    .out content.  Enforces § 9 forbidden pattern #2 mechanically.

    Allowed: filesystem-level checks (mtime / size / glob).
    Forbidden: re.search / re.match against `.out` body — must go
    through detect_parser().parse() instead.
    """
    src = (
        Path(__file__).resolve().parent.parent.parent
        / "molbuilder" / "jobs" / "decoder.py"
    ).read_text()
    # Heuristic: the decoder may regex over .fdf content (engine body
    # summary) but must not open any *.out files directly.  Confirm
    # neither `.out)` (path open) nor `out_path.read_text(` appear.
    assert "out_path.read_text(" not in src, (
        "decoder.py opens .out content directly; must go through "
        "detect_parser().parse()"
    )
    assert "read_text" not in src or src.count("read_text") < 8, (
        "decoder.py has too many read_text calls; should only read "
