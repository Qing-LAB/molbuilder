...
Node integration test: loads the REAL render-pipeline + render modules and drives
...
contract:
  ...
  * §14.0/§14.2 -- isolate FILTERS the RENDER list (a derived, display-only view); the STORED
...
getUnitCellInfo, §14.4); the harness names it ``data`` to match.  (k-grid tiling is gone:
k-grid is a reciprocal-space sampling knob, not a real-space repeat.)
"""
import json
import shutil
import subprocess
...
# same source the panel lists, §14.0).  The DATA MODEL supplies the RESOLVED cell.
# getUnitCellInfo().value is NON-NULL with isDefault:true -- i.e. a cell-less molecule's
# resolved bbox (§14.4).
_HARNESS = """
    const calls = [];
    const handle = { setStructure: (o) => calls.push(o), getAtomCoords: () => [[0,0,0],[1,0,0]] };
    const dataSubs = [], storeSubs = [];
    let storeState = { indices: [], isolate: false,
