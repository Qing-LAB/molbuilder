...
REQUIRED for the thermostat: without it SIESTA defaults the target to 0 K and the run QUENCHES instead of equilibrating.""",
    ...
    psml_lib: Optional[str] = field(default=None, metadata={
        ...
        "help":       "Path to a directory of .psml pseudopotential "
                      ...
                      "path, ``~/...``, or a relative path.  A "
                      "relative path is tried against the calculation "
                      "folder first, then against the ``projects/`` "
                      "tree the calculation lives in (found by walking "
                      "up from the calculation folder -- so just "
                      "``pseudopotential`` finds "
                      "``projects/pseudopotential/`` from anywhere "
                      "inside the tree, whatever directory you run "
                      "from), then against ``<cwd>/projects/``.  Tip: use "
