...
REQUIRED for the thermostat: without it SIESTA defaults the target to 0 K and the run QUENCHES instead of equilibrating.""",
    ...
    psml_lib: Optional[str] = field(default=None, metadata={
        ...
        "help":       "Path to a directory of .psml pseudopotential "
                      ...
                      "Where a relative path is measured FROM is decided by how you s"
                      "pell it: a bare name like ``pseudopotential`` means the ``proj"
                      "ects/`` tree this calculation lives in -- found by walking up "
                      "from the calculation folder, so it works from anywhere inside "
                      "the tree, on any machine -- while a path starting with ``./`` "
                      "or ``../`` means \"from this calculation\" and is measured from "
                      "the calculation folder.  Do NOT write the ``projects/`` prefix"
                      ": the walk-up already finds the tree, so ``projects/pseudopote"
                      "ntial`` looks for a ``projects/`` inside it.  Tip: use the fil"
