...
def render_spectra_script(struct: Structure, cfg: SpectraConfig) -> str:
    ...
    """
    lines: List[str] = []
    lines += _emit_header_docstring(struct, cfg)
    lines += _emit_imports(cfg)
    lines += _emit_constants(struct, cfg)
...
def _emit_header_docstring(struct: Structure, cfg: SpectraConfig) -> List[str]:
    ...
    """
    # Avoid a top-of-module circular import (methods imports
    # nothing back from this module, but render_methods_md pulls
    # in the engine registry which pulls this module in turn).
    from .methods import render_methods_md
    from .pyscf_engine import PySCFSpectraEngine

    methods_md = render_methods_md(
        cfg,
        engine=PySCFSpectraEngine,
        struct=struct,
    )
...
def _emit_constants(struct: Structure, cfg: SpectraConfig) -> List[str]:
    ...
    # Compute the bibliography keys by running render_methods_md
    # once more (cheap) and extracting them.  Keeps the in-script
    # list of keys consistent with the docstring above.
    from .methods import render_methods_md, extract_citation_keys
    from .pyscf_engine import PySCFSpectraEngine
    keys = extract_citation_keys(
        render_methods_md(cfg, engine=PySCFSpectraEngine, struct=struct)
    )
    out.append(f"BIBLIOGRAPHY_KEYS          = {list(keys)!r}")
    out.append("")
    ...
    out.append("METHODS_TEXT = ")
    methods_md = render_methods_md(cfg, engine=PySCFSpectraEngine, struct=struct)
    # Build the triple-quoted string carefully; escape any """ inside.
    escaped = methods_md.replace('"""', "'''")
    out[-1] = "METHODS_TEXT = \"\"\"" + escaped + "\"\"\""
    out.append("")
    out.append("MOLBUILDER_VERSION = 'spectra-v1'  # placeholder; filled by ")
    out.append("                                   # SpectraResults consumer")
    out.append("")
    return out
