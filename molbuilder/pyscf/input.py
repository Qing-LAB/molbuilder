...
def _emit_stages_loop(cfg: PySCFConfig,
    ...
    if v:
        ...
        out.append("#")
        out.append("# Per-stage non-convergence policy ('on_nonconvergence'):")
        out.append("#   'proceed'  -- accept whatever partial geometry geomeTRIC")
        out.append("#                 produced when max_steps ran out; hand off")
        out.append("#                 to the next stage.  Warm-up tiers.")
        out.append("#   'continue' -- retry this stage's optimize() on the same")
        out.append("#                 targets up to 'continue_retries' more")
        out.append("#                 batches (total budget =")
        out.append("#                 max_steps * (1 + continue_retries)).")
        out.append("#                 If still not converged, raise.")
        out.append("#   'halt'     -- raise RuntimeError + stop on the first")
        out.append("#                 failure.  Publishable / tight tiers.")
        out.append("# The LAST enabled stage is FORCED to 'halt' regardless of")
        out.append("# its declared value -- the script's contract is to produce")
        out.append("# a converged final geometry; no user knob can silently")
        out.append("# ship a non-converged answer.")
    out.append("STAGES = [")
    for i, s in enumerate(enabled):
        _is_final = (i == len(enabled) - 1)
        out.append("    {")
        ...
        out.append(f"        'on_nonconvergence': {s.on_nonconvergence!r},")
        out.append(f"        'continue_retries':  {int(s.continue_retries)!r},")
        # ``is_final`` is derived from position at generation time and
        # baked into the literal so a human reading the script sees
        # which row is the "must-converge" tier without having to count.
        out.append(f"        'is_final':          {_is_final!r},")
        out.append("    },")
    out.append("]")
    out.append("")
    # Inline helper: factors the per-stage optimize() call so the
    # 3-policy dispatch below stays readable.  Closes over mf +
    # _molwatch + _FROZEN_CONSTRAINTS_PATH from the enclosing
    # scope.  ``_hard_fail`` controls geomeTRIC's
    # assert_convergence: True raises on max_steps-exhaustion,
    # False returns the last accepted line-search geometry.
    if v:
        out.append("# Inline helper: one place that calls optimize() so the")
        out.append("# 3-policy dispatch below stays readable.  Closes over mf")
        out.append("# (+ _molwatch + constraints path when configured).")
    out.append("def _mb_run_stage_opt(STAGE, _hard_fail):")
    ...
    # The LAST stage is forced to 'halt' regardless of its declared
    # policy -- the script's contract is to produce a converged
    # final geometry.  ``_policy`` captures the effective rule.
    out.append("    _policy = ('halt' if STAGE['is_final']")
    out.append("               else STAGE['on_nonconvergence'])")
    out.append("    if _policy == 'proceed':")
    out.append("        # Warm-up tier: take whatever geomeTRIC produces.")
    out.append("        # max_steps-exhaustion is NOT raised (assert_convergence")
    out.append("        # = False); mol_eq is the last accepted line-search step.")
    out.append("        mol_eq = _mb_run_stage_opt(STAGE, _hard_fail=False)")
    out.append("    elif _policy == 'halt':")
    out.append("        # Production / final tier: hard-fail on non-convergence.")
    out.append("        # The RuntimeError surfaces with geomeTRIC's diagnostic.")
    out.append("        mol_eq = _mb_run_stage_opt(STAGE, _hard_fail=True)")
    out.append("    else:  # 'continue'")
    out.append("        # Extend this stage with up to 'continue_retries'")
    out.append("        # additional max_steps batches.  Each attempt uses")
    out.append("        # _hard_fail=True so we can catch the RuntimeError and")
    out.append("        # decide retry vs raise; the FINAL attempt always")
    out.append("        # raises on failure (semantic: 'continue then halt').")
    out.append("        _budget = 1 + STAGE['continue_retries']")
    out.append("        for _attempt in range(_budget):")
    out.append("            try:")
    out.append("                mol_eq = _mb_run_stage_opt(")
    out.append("                    STAGE, _hard_fail=True)")
    out.append("                break")
    out.append("            except RuntimeError as _e:")
    out.append("                if 'not converged' not in str(_e).lower():")
    out.append("                    raise   # genuinely different error")
    out.append("                if _attempt == _budget - 1:")
    out.append("                    raise   # exhausted -> fall through to halt")
    out.append("                print(f\"WARN: stage {STAGE['name']} did not\"")
    out.append("                      f\" converge in {STAGE['max_steps']} \"")
    out.append("                      f\"steps; retrying with same targets \"")
    out.append("                      f\"({_budget - 1 - _attempt} \"")
    out.append("                      f\"attempt(s) remaining)\")")
    if v:
