...
def _emit_stages_loop(cfg: PySCFConfig,
    ...
    if v:
        ...
        out.append("#")
        out.append("# 'assert_convergence' is FALSE on every stage except the")
        out.append("# last.  Rationale: earlier stages are warm-up tiers")
        out.append("# (intentionally loose), and a stage1 that exhausts its")
        out.append("# max_steps without hitting gmax should hand off to the")
        out.append("# next stage rather than RuntimeError-killing the whole")
        out.append("# run.  The FINAL enabled stage keeps the geomeTRIC")
        out.append("# default (True) so a real production-tier non-convergence")
        out.append("# surfaces -- that's the user's signal the SCF is wrong")
        out.append("# or the geometry needs a looser-stage prefix.")
    out.append("STAGES = [")
    for i, s in enumerate(enabled):
        ...
        out.append(f"        'assert_convergence': {_is_final!r},")
        out.append("    },")
    out.append("]")
    out.append("")
    ...
    out.append("        assert_convergence    = STAGE['assert_convergence'],")
    if emit_constraints:
