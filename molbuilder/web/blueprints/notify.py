...
alter a job.  So the worst a stolen key buys is noise in a file.  **No tab
reads that file** -- this sentence said "a log one tab reads" until
2026-08-31 and no such tab has ever existed; the reports are read with
`tail`, and a claim that they are read anywhere else made the boundary sound
narrower than it is.

**Four gates, and only one of them is the control.**

1. *Has anyone enabled this?*  The blueprint is registered only when the
   key file exists and carries a route (`app.py`).  **That file IS the
   switch**: it took two `molbuilder.json` settings until 2026-08-31 --
   ``notify_keys_file``, a path to a file molbuilder had itself written, and
   ``notify_route``, a copy of a segment molbuilder had itself issued -- and
   both are now retired and refused (`run-reports.md` § 4.3).  No file, no
   route in it, no listener.
