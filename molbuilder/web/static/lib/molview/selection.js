/* MolView — HOW A USER PICKS ATOMS: the panel beside the 3D window, its rows, and the
 * click-to-select wiring in the window itself.
 *
 * Contract: docs/web/molview.md § 9.5.  The selection is the truth; click and filter are
 * two EDITORS of it, and switching between them does not disturb what is already selected.
 * Click mode edits atom by atom, entirely in the browser.  Filter mode composes a query the
 * user explicitly applies, the SERVER evaluates it, and applying it replaces the selection —
 * MolView holds no matching logic (§ 2: one place decides what a structure means).
 *
 * Both halves are readers of one store and never of each other: the panel writes the
 * selection, the adapter writes the selection, and the store tells them both.  That is why
 * they are one file — one concern, two hosts (the strip, and the 3D window).
 *
 * Writing a LABEL is reached from here and is not like its neighbours: it changes the
 * structure (§ 9.5, § 6.6), so it is gated with every other truth change (§ 9.4).
 *
 * Assembled 2026-07-30 from selection/panel.js + selection/viewer-adapter.js.  Bodies
 * unchanged.  Its DOM still comes from templates/_selection_panel.html; bringing the markup
 * in-module is plan Phase 6.
 */
"use strict";
...
/* ── The panel beside the viewer — was selection/panel.js ────────────────────── */
...
/* ── Click-to-select in the 3D window — was selection/viewer-adapter.js ──────── */
