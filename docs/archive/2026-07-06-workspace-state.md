> **ARCHIVED 2026-07-06.** Closed migration log (Phases 1–9 all shipped). The live
> model + persistence contract is [`../protocols/workspace-contract.md`](../protocols/workspace-contract.md)
> (the MolView + Workspace core contract). Kept for history / the *why*; not policy.
> (Contains a since-superseded endpoint reference in §5.1 — the live wire shape is in
> the contract + [`../protocols/web-api.md`](../protocols/web-api.md).)
