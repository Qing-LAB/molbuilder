...
| **D** | **T2: one home per component.** `.card`, `.status`, `header .tagline` — the allowlist entries. Each page's variant either scopes under its own root or becomes a named variant. **One value to settle first:** `.card`'s own padding is `var(--space-md) 18px 18px`, and 18 is neither a hairline nor a dimension, so the app's canonical surface is off the 4px grid `ui-contract.md` § 4 declares — everything nested in a card inherits the offset. Moving it to `--space-md` shifts every page by 2px, which is why it is a step of its own rather than a side effect | shrinks the guard's allowlist to zero |
| **E** ◐ | **T3 per page**, one page per commit: tokens → namespace → residue. `modify`'s **values** pass and are the worked example — but its **composition** does not, measured in a browser 2026-08-30: three card titles on the page at three different left edges, six left edges inside one card, and one typeface spelled five ways (`plans/modify-redesign-plan.md` § 5). **A token pass is not a layout pass**, and this row said "done" for four weeks on the strength of one. `modify` finishes under that plan's item 5, which also renames `.card-header` and `.ghost` — both written by `modify.html` and by nothing else — out of the shared namespace, retiring two of D's allowlist entries. Then `spectra`, `structure-optimization`, `transport`, `results`, `documents` | independent, and a browser check per page is cheap |
