...
| `2026-08-16-task-setup-plan.md` | Superseded — the shared tab that finishes a description, planned under the working name *Task Setup*. The name is settled (**Job Prep**) and the design is agreed, so the plan became a second description of it. Its § 7 operations table — the nine edits that can each silently destroy a value — migrated in full before the move, as did § 3.1 (*holds no state of its own*) and § 4 (*"is this a calculation?" is answered once, by `checkpoint.py::_is_bundle_root`*). It carried two retired rules to the end: *"a one-stage calculation never opens this tab"* (`stages.md` § 6.5 now: a job always has at least one stage) and a `base:` key removed from `task.json` on 2026-08-07 | **`web/job-prep.md`** (the design, entire) |
