...
| **P2 · the tab substitutes** | the form from the catalogue; Send-to-Task-setup; the compute cards rewired | ✅ **code + pins DELIVERED 2026-08-20** — the tab renders `GET /api/build/schema/pyscf?calculation=vibration` through the shared renderer (the ES lock is schema-driven now — the old hardcoded ids had **never matched**, so the fade first works here); Generate/Save/methods/prior cards replaced by Send; the sender extracted to `lib/task-handover.js` (one door, two tabs — `/structure-optimization` delegates); auto-detect repointed and green through a real chromium (`net_charge`/`spin`/`method` land by name); the **byte-compat bar is pinned**: `test_the_browser_hand_over_writes_the_cli_s_files` proves the browser's template + structure pair byte-identical with `describe`'s and the description heads agreeing on id/kind/structure; the relaxation chip (data-driven fourth dot) + the § 2b thermo tab (G/H/T·S curves + decomposition bar, deck computes / viewer draws) landed with JS pins. ✅ **the bar ran live 2026-08-21** (after the user's server reload): water driven from the browser — sidebar pick → Load → auto-detect (RKS/0/0 by name) → Send (the cell gate's vacuum notice HELD the navigation, then four files on disk) → Task setup read the hand-over, proposed the `freq` ladder + `calculation: vibration`, saved on the Flat pick with a checkpoint note → `prep` + `submit direct` ran the deck → the Results tab, live: **four green dots with Relaxation first**, the meta row "complete — 3 steps, max |F| 2.0e-5 Eh/Å", and the **Thermochemistry tab drawing 3 curve traces + 4 decomposition bars** with the regime note.  Modes 1638.9/3791.2/3886.4 cm⁻¹; IR 55.5/4.9/27.0 km/mol **and** Raman 6.8/76.9/36.8 Å⁴/amu from ONE run (D1 working); rrho ZPE 0.02122 Eh, 30-point grid |
| **P3 · removal** | `render_script`/registry path, the two old routes, `SpectraConfig`'s form metadata, the in-deck `compute_frequencies` trio, the IR banner | ✅ **DELIVERED 2026-08-21** (three commits): the emitters MOVED to `pyscf/vibration_emitters.py` (git mv; the lift stayed a move) and `render_spectra_script` / `pyscf_engine` / `engine_base` / the two routes died around them; the banner states BAND-LEVEL VALIDATED (and the doubled "PySCF (PySCF (…))" header fixed); the in-deck trio retired with `temperature_K`/`pressure_atm` gaining `calculations = ["vibration"]` (the D2 re-homing finished); SpectraConfig keeps ONLY the validation vocabulary (196 form keys stripped, 19 range/validate/choices/pattern kept); the tab's dead Generate/Save/Methods half (~18.5 kB) swept with its els/state/CSS. **The removal's own catch:** the deck's science gate had silently SKIPPED since P1 — `validate()`'s dispatch is type-keyed and the deck's view is an adapter — found when the registry import broke; the science moved whole to `validation/spectra.py`, the deck composes it BY NAME, and `test_vibration_render_gate.py` pins refusal + advisory + registry parity (mutation-proven red).  `script-preparation.md`'s table shows Spectra ✅ across; every remaining `spectra/` module serves the artifact |
