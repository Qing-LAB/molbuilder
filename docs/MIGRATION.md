...
| 4 | `engines/` (~8): SIESTA, PySCF, transport/TranSIESTA, builders, GPU recipe; `transiesta-workflow` + `transport-guide` move in | **in progress** (2026-07-26) — shape = `overview` map + peer docs (`siesta`/`pyscf`/`transport`/`builders`/`siesta-gpu`/`tuning`), overview composed last. `siesta.md` + `pyscf.md` + `transport.md` + `builders.md` **done** (each fresh-eyes audited + enrichment-passed; each caught real source-vs-code drift). `transport.md` consolidated 4 sources + closed the `region-labels` split (transport half); `builders.md` reconciled the synthesis contract (audit found zero factual errors; doc corrected two stale source facts). Sub-order: siesta → pyscf → transport → builders → **siesta-gpu** (next) → tuning → overview. Deferred split-halves land here: `region-labels` (transport), `sidecar-contract §1–10` (→ overview boundary contract), `scientific-validation §7.5` GPU (→ siesta-gpu). |
...
| `engines/siesta.md` | engines/siesta.md | merged-into engines/siesta.md | Wave 4 (2026-07-26). The `.fdf` emitter contract migrated in full + reconciled vs `siesta/input.py`, `config/siesta.py`, `makov_payne.py`, `memory.py`, `stages.py`. Line numbers verified (`render_fdf:329`, `convert:1486`, `SiestaConfig:114`, `SiestaStageSpec:1235`, presets `:1414`, …). Added E5 two-surface framing + Mermaid + worked examples; mapped the two sibling modules the source omitted (`makov_payne` post-process, `memory` estimator → pointers). Cross-refs: GPU/ELPA *build* → `siesta-gpu.md` (this wave); stage-ladder *scheduler* execution → execution/; SIESTA half of `optimization-tuning` → `engines/tuning.md` (this wave). |
