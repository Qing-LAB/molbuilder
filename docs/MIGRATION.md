...
| 4 | `engines/` (~8): SIESTA, PySCF, transport/TranSIESTA, builders, GPU recipe; `transiesta-workflow` + `transport-guide` move in | **in progress** (2026-07-26) — shape = `overview` map + peer docs (`siesta`/`pyscf`/`transport`/`builders`/`siesta-gpu`/`tuning`), overview composed last. `siesta.md` **done**. Sub-order: siesta → pyscf → transport → builders → siesta-gpu → tuning → overview. Deferred split-halves land here: `region-labels` (transport), `sidecar-contract §1–10` (→ overview boundary contract), `scientific-validation §7.5` GPU (→ siesta-gpu). |
