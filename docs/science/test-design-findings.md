...
| ~~`test_add_slab.py::test_the_same_holds_on_the_two_period_surfaces`~~ | ~~`moved > 0.1`~~ | **CLOSED 2026-09-09.** Measured, and the derivation in this row was half right: (100) is `a/2`, but (110) is **`a√6/4`** — the magnitude of the `(a/2√2, a/2)` ABAB shift, not `a/(2√2)` alone. Both now asserted at `abs=1e-3` like the (111) sibling; a 1% wrong lattice constant fails seven tests and a wrong stacking period fails it directly. |
...
| ~~`test_modify.py::test_electrode_lattice_constant_override`~~ | ~~`e_extent > d_extent + 0.5`~~ | **CLOSED 2026-09-09.** The ratio is exact — `extent(5.0)/extent(default) == 5.0/a`, measured 1.226031092 on both sides — so it is asserted at `rel=1e-9` with the default read from `modify._get_fcc_lattice()` rather than retyped. A lattice constant applied 2% off now fails; under the old assertion it passed. |
