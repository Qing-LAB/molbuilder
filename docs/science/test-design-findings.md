...
|---|---|---|
| `test_makov_payne.py::test_q_plus_one_vacuum_L15` | `1.35 < dE < 1.40` | ±1.8%. The comment *derives* 1.362 from `MADELUNG_CUBIC`/`BOHR_ANGSTROM`/`HARTREE_EV`, but the assertion never uses them — **a 1% error in the Madelung constant passes.** Assert against the imported constants at `rel=1e-9`. |
| `…::test_script_runs_on_synthetic_out` | `-122.5 < corrected < -121.5` | ±0.5 eV on a 1.362 eV correction: sign and rough magnitude pinned, a 30% error not. |
...
| `test_add_slab.py::test_the_same_holds_on_the_two_period_surfaces` | `moved > 0.1` | Its (111) sibling compares against `a/√6`. The (100)/(110) steps are equally derivable (`a/2`, `a/(2√2)`); asserting them would state the physics instead of "something moved". |
...
| `test_modify.py::test_electrode_lattice_constant_override` | `e_extent > d_extent + 0.5` | Extent is linear in `a`, so this passes for **any** `a` above the default. Assert the ratio, or the nearest-neighbour distance `a/√2`. |
