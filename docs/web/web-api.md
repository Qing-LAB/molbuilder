...
| GET `/api/task-setup/folder` | **What is this folder?** — Task setup's one per-directory answer, so the page is a function of the folder rather than of twelve calls landing in whatever order they land ([`task-setup.md`](?doc=web/task-setup.md) § 2.1). Carries the description or the hand-over (and so the `mode`), the template's values, the provenance block, the attempts on disk, and the folder's file names. **It names its own subject** in `dir`, and a consumer that has moved on discards it — `calcdir.json`'s rule (`project-layout.md` § 1.4a) applied to the wire, and the half that clearing state cannot cover. Composed from the same helpers `template-values`, `resolved` and `attempts` return, so the door and those routes cannot disagree; one part failing carries its own `error` rather than failing the answer |
...
| POST `/api/task-setup/attempts` | How many attempts each stage has ON DISK, from the DECLARED shape. Body `{dest}` — a folder, unlike `prep-plan` which reads only the document. The page worked this out itself until 2026-09-08 and needed four of our rules to do it — the stage token, the `run-<n>` pattern, the flat `_<token>-run` form, and a branch between the last two chosen **by looking at the disk**, which [`project-layout.md § 4.5`](?doc=execution/project-layout.md) forbids by name: the shape is declared, never inferred |
...
| GET `/api/results/dir` | **What is in this run directory, and which file should open?** — the HTTP surface over `parse.dirs`' front door (`model/parse.md` § 5.5). Per DIRECTORY: `place` (container or run, from `calcdir.json` — `project-layout.md` § 1.4a), `openable` (the door's own pick, so the page defaults to what the CALCULATION produced), `attempts` (the search trail, which is the body of the refusal when nothing matched), `status`, `engine`. Per FILE: `role`, `label` and `stage` read back with the label the deck states (`runfiles.parse`), and `parser` — the registry's verdict, `null` when nothing can read it. It exists because the browser is the one consumer that cannot import Python and was deciding all of that from filenames: measured over 110 real run directories, the guess differed from the door on 18 of 96, offered 13 files no parser can read, and hid 155 it handles |
...
| GET `/api/transport/describe_attempt` | Classify a picked directory against [`engines/transport.md`](?doc=engines/transport.md) § 3.1's FILE condition: `?path=` (tree-relative) → the form (`relaxation` \| `structure` \| null with the refusal naming the missing file), the contract lane (`cited` \| `open`), the honest convergence state, the deck summary, the labeled `structure` envelope the viewer installs — answered **whether or not the citation composes**, so a refusal is read over the junction it is about — and `fix`, a word the tab acts on (today `"swap_electrodes"`; see `/api/transport/swap_electrodes`) |
...
| GET `/api/transport/schema` | The transport tab's override form, built from `TransportConfig` and filtered to the **override lane**: Transmission / NEGF / Runtime knobs that ride the device stage's bag (`engines/stages.md` § 6.2). Contract fields, the always-sealed set and the unresolved set are hidden, and a section the filter empties (System, Electrodes) is dropped whole; order follows `TransportConfig._form_section_order`. The bias is not here — it is card 4's own input, a describe-level fact beside the citation |
