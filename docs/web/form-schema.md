...
|---|---|
| `optional` | *unset* is a real state and the control must offer it. It is **not** inferable from `null_label`: of **14** optional items only **10** carry one, so four would silently lose their *(auto)* option |
