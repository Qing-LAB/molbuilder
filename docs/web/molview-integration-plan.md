...
|---|---|---|
| `factsForRequest()` | `structure-optimization/viewer.js` | **`exportFile()`** for a request; `getStructure()` to look at the structure. § 9.3a settles which is which (2026-08-03): both are one read, and the difference is the words and the frame — `getStructure` is the master copy whole in the module's own vocabulary and *every* frame; `exportFile` is the same facts in the server's vocabulary for the frame on screen. Reading this row as "use `getStructure` for requests" is what produced a hand-built envelope with the cell under a key the receiver refuses |
