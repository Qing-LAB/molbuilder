...
| Slot provenance | `slot-provenance.json` at the transport calculation's root — which attempt the composed junction came from, with content hashes; part of the § 4.1 travelling copy (`transport-design.md`) | `molbuilder/slot-provenance@1` | `transport/compose.py` | `slot`, `citation`, `calculation`, `attempt`, `concluded`, `deck`+`deck_sha256`, `xv`+`xv_sha256` |
