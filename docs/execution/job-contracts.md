...
| `bench-result.json` / `bench-recommendation.txt` | the measured sweep, and the report you read | `summarize` | § 2 |
...
| Bench report | `<seq>_<stage>/bench/bench-recommendation.txt` — beside the record it is built from; **read by a person and by no code**: `summarize bench` writes what was measured and the `execution` block that would use it, and a run uses it only once someone has written that block (`project-layout.md` § 2.3.2). *(It was `run-config.toml`, `molbuilder/run-config@1`, an editable TOML `prep run` folded in, until 2026-09-02 — see `architecture.md` § 5.2.)* | *(plain text, no schema — it is prose)* | `jobset/summarize.py` | — |
