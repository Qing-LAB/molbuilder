...
| 32 | **324 bare citations of 32 archived-only documents.** Found 2026-08-10 while auditing one redundant import. The 2026-07 migration repointed ~319 *path* references and `tests/test_no_retired_doc_paths.py` has guarded paths ever since — but a docstring saying `slurm-integration.md § 4.3` carries no `docs/` prefix, so it matched no pattern and read exactly like a citation of a live contract. The guard now measures them (`python -m tests.test_no_retired_doc_paths`) and holds the count as a strict xfail. **The largest is `slurm-integration.md` — 51 citations, 18 distinct sections, across `runwrap`, `bench`, `runtime_config` and `jobset`.** Its ledger row says *merged into `job-system.md` § 5*, and the narrative did merge; the **specification did not**: archived § 7.5.1 (rank sizing, GPU pinning) is 124 lines against three mentions of `CUDA_VISIBLE_DEVICES` in the live tree, and archived § 10 (what makes a scheduler header complete) is 14 lines against one sentence. Next are `parse-module.md` (46) and `job-execution.md` (41). **Each document needs its own call:** REPOINT where the successor really carries the content, or UN-ARCHIVE where it does not — a document dozens of code sites depend on is a source of truth whichever directory it sits in. **Recommend un-archiving `slurm-integration.md`** as `execution/slurm-integration.md`, reviewed against the code. Not a remedy: rewriting the citations as explicit `docs/archive/…` paths, which the guard used to recommend — that makes live behaviour permanently specified by a document `docs/README.md` calls *"Not a source of truth"* | gates the `bench` fold-in, its largest consumer |
