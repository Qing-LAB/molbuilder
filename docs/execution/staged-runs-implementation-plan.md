...
| 33 | **The template's declaration format needs a Python floor decision.** `job-contracts.md` § 3.7 settles the marking: a **TOML inline table** in the engine's comments, carrying `kind=` from a closed vocabulary (`engine` · `deck` · `wrapper` · `produce` · `monitor`) so any layer filters on one token. It is chosen over a bespoke `key=value` grammar because that one cannot express a string with a space — which is exactly how the kind ended up as unparseable prose inside `anchor=` — and over JSON on readability, in a file `project-layout.md` expects a person to edit. **The parser is `tomllib`, standard library from Python 3.11; `pyproject.toml` declares `requires-python = ">=3.9"`.** So: raise the floor to 3.11 (already tracked as housekeeping), or take the `tomli` backport — the same parser under its pre-stdlib name. **Everything else about the format is settled; this is the only open piece** | P12 unit 6b, which cannot start until the declaration format is fixed |
