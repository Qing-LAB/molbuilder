...
| 2 | Is the **folder named by the id**? It removes a second name and makes a listing self-describing, at the cost of `bdt_au_relax_c6h4s2au38` where someone would type `bdt-relax` | P1 |
| 3 | Is a description **editable by hand**? It is JSON beside the decks, so it will be — which argues for the refusal rule being loud rather than tolerant | P1 |
| 4 | May **two enabled stages be identical**? Probably "warn": two decks differing only in name compute the same thing twice into the same warm state | P1 |
| 5 | Where does a per-stage **`continue_retries`** travel — `Resources` gains a slot, or a stage gets a different road to its wrapper? | P2 |
