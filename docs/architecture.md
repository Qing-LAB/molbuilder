...
| Send a structure to a validating / emitting endpoint | **`molview.data.factsForRequest()`** — coordinates + labels + periodicity, read live | a page-local `state.xyz` mirror, or a server-side disk re-read | [`science/validation.md`](?doc=science/validation.md) § 4.1 (F1/F2) |
