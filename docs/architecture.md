...
| Parse an engine `.out` / dir / sidecar into typed data | `parse.registry.parse` / `parse_dir` / `parse_text`; `parse.dirs.job.run_status` (how a run dir is doing) | write a bespoke parser | [`model/parse.md`](?doc=model/parse.md); decoded-run view → [`execution/running-a-job.md`](?doc=execution/running-a-job.md) |
...
| `parse/` | L2 | unified **read stack** (File / Text / Dir parsers → typed `ParseResult`) | `parse.registry.{parse,parse_dir,parse_text}`; `parse.dirs.job.run_status` | [`model/parse.md`](?doc=model/parse.md) |
