...
| Build a calculation on a finished run (relax → transport) | **cite the attempt** — `jobset init --slot junction=<calc>@<stage>/run-N`; `prep` composes from the citation (`transport/compose.py`).  The handoff bundle retired 2026-08-29 | copy geometry ad hoc; re-grow a bundle writer | [`plans/transport-design.md`](?doc=plans/transport-design.md) § 4.1; [`execution/job-contracts.md`](?doc=execution/job-contracts.md) § 5 (the closure) |
