...
| [`ops/server-reload-plan.md`](?doc=ops/server-reload-plan.md) | plan | **Seeing a code change without a manual restart** (proposed 2026-08-03) — two halves that must not get one answer: **JS/CSS are only stale in the browser's cache** (templates re-read per request; static files too), so they need a **version on the URL**, not a restart; **Python** genuinely needs a fresh interpreter. A file watcher is rejected deliberately — Werkzeug's stat-polls every imported module once a second and fires on *any* mtime change, so a chunked editor write or a `git checkout` reloads against a momentarily inconsistent tree and the child comes up importing half a module. What is worth keeping from the reloader is not the watcher but the **process shape**: a parent that never imports application code and respawns a child on a sentinel exit — which is exactly what a route re-execing itself cannot give itself. So the trigger becomes a person: an admin-gated `POST /api/admin/reload` that answers first, then exits; the parent respawns; the browser polls the existing `/api/health` and reloads. **The gate is the part to decide first** — `admin_emails` ships empty and its own comment says that means *any logged-in session is admin*, so the button as drawn would hand every user a way to disconnect everyone; the route must not exist while the list is empty |
