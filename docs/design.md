...
doc index + the rules); [`plans/plan.md`](?doc=plans/plan.md) (open work);
...
| 2026-08-11 | **The run directory is defined by what it does** — it hands the engine everything it needs and holds everything it produces. Every file in it is *rendered* from the description or *copied* in; a stage directory means the science changed, a `run-` directory means you continued | [`execution/project-layout.md`](?doc=execution/project-layout.md) § 1.0, § 1.5b |
| 2026-08-11 | **One workflow, flexible in five directions** — surface, environment, shape, engine, kind — where **every axis is a value read at one point, never a branch**. The two breaches this row once named are closed: `bench`'s duplicate of `jobset` MERGED (the 2026-08-12 fold — benchmarking is `prep` whose parameters are a set), and `transport` was **decided** to be a separate kind rather than a gap (2026-08-11, user — it is not getting edges) | [`execution/architecture.md`](?doc=execution/architecture.md) § 0 |
| 2026-08-06 | **A project directory has two shapes, and which one is a field in the description** — *flat* (stages by filename suffix, attempts by output index, **warm files unsuffixed and shared**) and *hierarchical* (a directory per stage, per attempt). Neither is wrong: they differ in **where the history lives**. Flat keeps one state on disk and its history **in time**, in the checkpoint; hierarchical keeps every state **in space**, on disk. So a checkpoint is insurance in one shape and **the mechanism** in the other | [`execution/project-layout.md`](?doc=execution/project-layout.md) § 1, [`execution/checkpointing.md`](?doc=execution/checkpointing.md) § 8 |
...
| 2026-08-06 | **The browser writes a portable package; `prep` on the target finishes it.** Forced, not stylistic: `BlockSize` is bounded by the orbital count over the rank count and written *inside* the `.fdf`, and the eigensolver picks both the numerics and the conda environment — so a deck finished on a laptop is guessing. `prep` is a **hub you return to**, not step four of a line | [`execution/project-layout.md`](?doc=execution/project-layout.md) § 2 |
...
| 2026-06-25 / 06-23 | ~~**Staged-optimization parity**~~ — **reversed 2026-08-07**: the two ladders separate, because an engine config carries no stage list ([`engines/stages.md`](?doc=engines/stages.md) § 1.1). SIESTA's moves to `task.json`; PySCF kept its own while the SIESTA path was built. **Amended 2026-08-17 (user): PySCF's moves to `task.json` too** ([`engines/stages.md`](?doc=engines/stages.md) § 1.1a) — § 1.1's exception named its own gate (*"until the SIESTA path works"*) and the gate is met. This **completes** the 2026-08-07 direction rather than reversing it: that decision deleted the SIESTA engine config's stage list, and this deletes PySCF's. What stays separate is **execution** — PySCF's rungs share one SCF process, so its ladder is one deck and one job, which was always the real difference; where the ladder is *declared* never was. (SIESTA `SiestaStageSpec` mirrors PySCF; three-source preset drift gate) and the **SIESTA universal-fdf-keyword** fix (`MD.NumCGsteps` is universal; an L4 binary-in-the-loop test gates the silent-failure class) | [`engines/tuning.md`](?doc=engines/tuning.md), [`engines/siesta.md`](?doc=engines/siesta.md), [`engines/pyscf.md`](?doc=engines/pyscf.md) |
...
| 2026-06-17 | **HTTP four-bucket status semantics** (success / scientific-advisory / protocol error / server fault; the scientific-advisory case stays HTTP 200 + `ok:false`); ~~the **run-bundle handoff** (`POST /api/results/bundle`) closes the workflow-continuation gap~~ — **retired 2026-08-29**: citations replaced it (`job-contracts.md` § 5) | [`web/web-api.md`](?doc=web/web-api.md), [`execution/job-contracts.md`](?doc=execution/job-contracts.md) |
...
*plans* live in [`plans/plan.md`](?doc=plans/plan.md), never here (a contract holds
