...
| 2026-08-11 | **One workflow, flexible in four directions** — surface, environment, shape, engine — where **every axis is a value read at one point, never a branch**. Two things breach it today and are named rather than glossed: `bench` duplicates `jobset` (a merge), and `transport` chains three runs the `JobSet` model cannot describe (a gap) | [`execution/architecture.md`](?doc=execution/architecture.md) § 0 |
