...
**P3. Every operation is explicit and user-initiated.** No automatic commit. No wrapper hook. No background activity. The user chooses when to checkpoint, and that choice is the only thing that creates a commit. Running the wrapper without checkpointing first is a deliberate user choice — the prior state is the prior state, and there is no auto-recovery.

**P4. The wrapper is git-agnostic.** `runwrap.py` never invokes git. The run-script generated by the wrapper never invokes git. This guarantees SLURM compatibility — compute nodes do not need git access, network access, or any extra environment setup to run the user's script.
...
**P8. Git availability is a hard requirement.** Bootstrap fails if `git` is not on PATH. This is enforced once, at install time, not deferred to per-run failures.
...
| **L2**: orchestration | `molbuilder/checkpoint.py::Repo` | Init, user-invoked checkpoint, binary archive write/read. Wraps `subprocess.run(["git", ...])`. |
| **L2**: CLI | `molbuilder/cli.py::cmd_snapshot_group` (new) | `molbuilder snapshot {init,checkpoint,list,tag,diff,restore,branch,prune}` |
| **L3**: HTTP routes | `molbuilder/web/blueprints/checkpoint.py` (new) | Read-only endpoints + checkpoint/tag/branch/restore POST endpoints. |
| **L3**: sidebar UI | `molbuilder/web/static/lib/projects/checkpoint.js` (new) | The graph viewer, sensor badge, action menus. |

**Explicitly NOT in the surface**: `runwrap.py` is unchanged. The wrapper does not call into `checkpoint.py`. This is load-bearing for P4 (SLURM compatibility).
...
### 4.2 Phase 1 — Working-dir init (user-driven)

**Trigger**: `molbuilder snapshot init` (CLI) or the sidebar "Initialise run history" button (UI).
...
git add .gitignore .binsnapshots/.gitkeep
git commit -q -m "molbuilder: initialised checkpoint history for <dir>"
```
**Result**: a one-commit repo with only `.gitignore` and the empty binary-archive dir tracked. The user's existing files are **not** auto-staged — they will be committed when the user runs the first explicit checkpoint (Phase 2). This avoids the surprise of "I just init'd and now my entire dir is in a commit I didn't make."

**Refusal**: if the directory has subdirectories that are themselves working dirs (contain `.fdf` / `.py` / `.run.sh`), Phase 1 refuses with one message naming them — per P5, each lowest-directory is its own repo, and a higher-level init would violate the scoping rule.
...
| 6 | `envs/recipes.py` — add `git` to host env | ~30 min | — |
| 7 | Bootstrap preflight (`git --version`) | ~30 min | 6 |
