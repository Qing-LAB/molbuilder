...
| 8 | Collapse persistence: delete `sessionStorage["modify-state"]` + `sessionStorage["molbuilder.structure_canvas"]`; write only `sessionStorage["molbuilder.workspace.v1"]`. | client | medium | **⚠️ partially shipped 2026-06-08.**  What landed: the dispatcher writes the unified snapshot under `molbuilder.workspace.v1` (schema v1) on every state change (debounced 100ms) plus a final flush on `pagehide`.  Eager subscribe-on-mount so persistence fires regardless of whether a UI consumer subscribed.  Public `readPersistedSnapshot()` getter on `ws` for the restore path.  Pinned by `tests/test_workspace_dispatcher_js.py::TestPersistRoundtrip::{test_storage_key_is_v1, test_persist_writes_to_unified_sessionStorage_key, test_readPersistedSnapshot_returns_null_when_no_key, test_readPersistedSnapshot_returns_parsed_snapshot, test_readPersistedSnapshot_returns_null_on_schema_mismatch}`.  **What did NOT land:** the legacy per-store mirrors (`molbuilder.structure_canvas` from canvas-state, `modify-state` from modify viewer.js) are still actively writing — so a state change today fires THREE setItem calls + a pagehide writes all three.  The dirty-gated restore policy from cd9655e prevents stale-disk override on either mirror, so this is correctness-neutral but a real overhead violation of the migration's "ONE key" goal.  A follow-up commit needs to: (a) make canvas-state's `_persistToSession` a no-op, (b) make modify-tab's `saveModifyState` a no-op, (c) extend the dispatcher's snapshot to carry the modify-tab-specific extras (camera, show_axes, show_indices, rep — currently in modify-state but not in the dispatcher payload), (d) point `restoreModifyState` at `ws.readPersistedSnapshot()` instead of the legacy modify-state key. |
