...
| **A1** | ⚠ **NOT REPRODUCIBLE — re-measured 2026-09-13, and the row overstates it.** On CPython 3.14 both forms write the archive immediately (24 bytes on disk the instant the rotation returns) and neither leaks a descriptor (`/proc/self/fd` unchanged across a rotation). A ResourceWarning IS still emitted for that object under `simplefilter("always")`, traced to it by tracemalloc with no descriptor leaking, and it is unexplained rather than fixed. The explicit `with … as raw` landed anyway, because ownership of a descriptor this file opened should not depend on another library's finalizer — but it closed no measured defect. **Original claim:** `LogRoll._rotate` **leaks the archive's file descriptor.** `gzip.open` owned its fd; `gzip.GzipFile(fileobj=_open_private(...))` does not close `fileobj`, so the `.gz` flushes only when CPython's GC collects the writer, and any run under `-W error` fails. `serve_daemon.py:187-190`. **Fix:** `with _open_private(dst,"wb") as raw, gzip.GzipFile(fileobj=raw, mode="wb") as fout:` | `ResourceWarning: unclosed file <_io.BufferedWriter name=3>` | **RAN** |
