...
| **R12** | **Decisions for the owner**: (a) `script_generation.preactivate` is still accepted as an alias while every other retired key is refused by name (C-R7); (b) `load_channels(path=)` and `write_config_scope`'s project branch exist for tests only (C-R4/R5); (c) `BuildComponent.needs_cuda` and `Recipe.system_preconditions` have no reader (B-R4) -- recipe fields, so yours; (d) `repair`'s policy lives entirely in the surface (B-Y1), a refactor not a defect. | ⏳ open |
