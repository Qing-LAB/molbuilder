...
|---|---|---|---|
| **J1** | `builds.dispatch_into_env` -- env-framework § 5.6 / M1 say all three dispatches go through it, naming `_dispatch.run_in_env` as the third | `run_in_env` never calls it: re-spells the whole mamba-1.x fallback (the seen-gate, the stub check, three `subprocess.run`s) and flips the door's PRIVATE state by hand (`_builds._MANAGER_RUN_UNUSABLE["seen"] = True`). Cause: the door returns `(rc, output)`; the router's one caller (`_amber.py`, `tleap`) needs a `CompletedProcess` with `cwd`/`timeout`. A real need answered by a copy instead of by the door growing the shape. | ✅ **CLOSED 2026-09-13.**  The door grew the shape instead: `dispatch_into_env(cwd=)`, and `run_in_env` calls it -- quietly, addressed at `caps.env_prefix(name)` (M2), returning a `CompletedProcess` whose `stdout` is the door's one transcript; a timeout overrun raises `subprocess.TimeoutExpired` off the door's own kill line (`builds.timeout_tail`, one spelling).  The router's stub check, wrapper, `_MANAGER_RUN_UNUSABLE` poke and upward import of `install._env_prefix` are deleted; `run_tool` takes `cwd`/`timeout` in place of `**popen_kwargs`.  Test: through `run_tool` with a fake door -- the argv, the prefix, `cwd`, `timeout`, `sink=None`; mutants (bare `subprocess.run` by name; timeout not mapped) fail it. |
| **J2** | `runtime_config.write_config_scope` -- § 2.3's designated writer for `molbuilder.json` / `.molbuilder.json` | **Zero production callers.** `scheduler/probe.py:16` still says `probe --write` merges "via write_config_scope"; N4 moved that output to `environment.json` via `write_environment` and the docstring never followed. | ✅ **CLOSED 2026-09-20 — the premise expired.** `write_config_scope` now HAS a production caller: `cli.py:1506` writes the `auth` block through it, so it is no longer a door nobody sees and there is nothing to decide. The docstring half WAS still wrong and is fixed: `scheduler/probe.py` said `probe --write` merges into `.molbuilder.json` via this writer, when `jobset/_cli.py:3702` writes `environment.json` through `write_environment`. |
...
| **J5** | `install.remove_env_cmd` -- "The ONE spelling" (M3) | `envs/_cli.py:1373` spelled the line by hand with its own copy of M3's reasoning (and would print `None env remove …` with no manager detected). `builds.py:1236` prints a literal `` `conda env remove` `` inside `preflight`, which has no manager in scope. | ✅ **CLOSED — re-derived 2026-09-20.** Both sites. The literal `` `conda env remove` `` is gone from `builds.py`'s preflight message; the only surviving occurrence of that string in the file is a comment about `XDG_CACHE_HOME` (`:1244`), which names no command to a user. |
...
| **J7** | the placement table -- the contract for expected modes | `web/blueprints/notify.py:172-173` declares its own `REPORT_MODE = 0o600` / `REPORT_DIR_MODE = 0o700` and `os.chmod`s after creating (the retired fix-up-after shape), while the table's `reports/` row says mode **None**, "no credential". | ✅ **CLOSED — re-derived 2026-09-20, resolved in the TABLE's favour.** `web/blueprints/notify.py` no longer declares `REPORT_MODE` / `REPORT_DIR_MODE` and no longer `chmod`s: it asks `config_dir.reports_dir()` and writes. So the code no longer enforces a mode no contract states, and the `reports/` row's `None` is now the only statement of the rule. |
...
| **L2** | ⛔ **WITHDRAWN 2026-09-13** (user: a scientific report, nothing to guard; on HPC a home directory is the user's own).  The 0600/0700 rule for `reports/` -- from a 2026-08-27 review note -- is removed: row back to not policed, plain `RotatingFileHandler`, the two mode tests retired, § 4.1a and the tree corrected.  `open_private`'s text/binary mode and the one pair of mode names stay.  *(What stood here for the afternoon:)*  The row says private (`reports/` 0700, and a new `a run-report record` row, `*.jsonl*`, 0600); the handler is a `RotatingFileHandler` whose `_open` goes through `serve_daemon.open_private`, so the first file and every rotated one are 0600 from the first byte; `open_private` gives text or bytes by the mode asked (`"a"` vs `"ab"`), which is what let a `logging` handler use it; the directory through `ensure_private_dir(tighten=True)`.  Test: `LOG_BYTES` shrunk, three reports, the rotated file is 0600; mutant (base `_open`) fails it and the first-file test.  **Run-report files lose their mode on rotation.** `notify.py:279-290`: `RotatingFileHandler` opens the file at the umask mode in its constructor, one `os.chmod(0o600)` runs after, and on rollover the handler creates the next file at the umask mode with nothing re-tightening it.  The comment says "tighten it here AND after"; there is no after.  And the placement table's `reports/` row says mode None ("no credential") while this code enforces 0700/0600 -- contract and code disagree about what these files are. | decide the row (I would say private: they name the user and carry results); the handler's `_open` goes through `serve_daemon.open_private`, which is the door § 2.3 names for an appended log | yes -- the table row |
...
| **L5** | ✅ **CLOSED 2026-09-13.**  Guarded on `SUPERVISED_ENV`.  **`serve foreground` prints the config warnings twice.** `cli.py:2221-2237` run in the parent and again in the re-exec'd child (`SUPERVISED_ENV=1`); the comment says "so it appears once". | guard on `SUPERVISED_ENV` | no |
| **L6** | ✅ **CLOSED 2026-09-13** with D1.  The wizard no longer touches the session key (the server's `_install_secret_key` is its one creator); `--output`, the `cd` branch and the "replacing it whole" path for an unreadable file are gone.  Test: a key the server made comes out of the wizard byte for byte.  **`auth-setup` overwrites the session key on every run, and gives `cd` advice for a file it says is unread.** `:1708-1709` regenerates `secret_key` (every session dies; the docstring calls this "idempotent EXCEPT") while `web/auth._install_secret_key` already creates one when absent -- with a different encoding (`token_bytes` vs `token_urlsafe`).  `:1742-1745` prints `cd <dir>` when the output landed in cwd, a location the same docstring says is NOT read.  `--output` is the `--keys-file` class this file retired at `:1984-1991`. | the wizard stops touching the session key; `--output` and the cwd branch go; the write goes through `write_config_scope` (D1) | with D1 |
| **L7** | ⏸ **DEFERRED 2026-09-13** (user: the static option has built fine; GPU is an option the benchmark decides, `MOLBUILDER_CUDA_CC` is the knob and is now documented in `installation.md`).  The recipe table is built at import, so deferring the probe means a lazy table -- not worth it for one `nvidia-smi` call on GPU hosts.  **Every `molbuilder` invocation runs `nvidia-smi`.** `recipes.py:346-376` probes the driver at import; `cli.py:38` imports the envs group, which imports recipes, at module level -- so `--help` shells out with a 2 s timeout. | resolve `_CUDA_VERSION` when a recipe is asked for, not when the module loads | small design |
| **L9** | ✅ **CLOSED 2026-09-13.**  `remove_step_for(prefix=)`: by name, with `env remove --prefix <dir>` as the step's fallback when the probe's reading (handed to `run_install` as `env_state`) shows a directory the registry does not list; the pure plan stays by name.  BROKEN's remedy no longer promises `--clean` (the manager will not remove a directory without `conda-meta`); it names `rm -rf <dir>`.  Test: an ORPHAN reading, `-n` refused by the fake manager, `--prefix` taken, RECOVERED, run succeeds; mutant (no fallback) fails it.  **`--clean` cannot remove an ORPHAN, and the ORPHAN hard-stop prints `--clean` as the remedy.**  `remove_step_for` addresses by `-n` (`install.py`), and an ORPHAN is by definition a directory conda's registry does not list -- `env remove -n` finds nothing.  Removing it needs `--prefix <dir>`, which only the probe knows (`prefix_from_fs`) and the pure plan cannot.  Found while fixing L1; not fixed there.  Shape: the removal step carries the probed directory as a declared alternative (`--prefix`), the way a pip step carries its index fallback. | a REMOVE step with a `--prefix` fallback when the probe found a directory the registry does not list | small design |
...
| | finding | fix shape |
|---|---|---|
| **Y1** | ✅ **CLOSED 2026-09-13.**  `recipes.effective_name` (+ `HOST_ENV_ENV`, `HOST_CATEGORY`); `install` no longer imports `doctor`, and `doctor` imports `install` at module level like any other module.  `_effective_name` -- name resolution from config and an env var -- lives in `doctor.py` (the audit) and is imported by `install.py` at module level (`:56`), which is why `doctor` defers its own imports of `install` (`:515-518`, "the cycle is deliberate"). | move it beside `Capabilities.env_for_category`; the cycle disappears |
| **Y2** | ✅ **CLOSED 2026-09-13.**  `machine_config_warnings` lives in `placement` (shadow + findings); `runtime_config.placement_warnings` is gone and `runtime_config` imports nothing upward.  `placement` and `runtime_config` import each other lazily (`placement.py:73`, `runtime_config.py:1323`).  `machine_config_warnings` is an audit sum. | let it live in `placement`; `runtime_config` then imports nothing upward |
| **Y3** | ✅ **CLOSED 2026-09-13.**  The claim is dropped: `record.py` says it is not shipped (`runwrap.MONITOR_COMPANIONS` is) and what it imports; `local_facts` states its real reason (it reads `runtime_config`, a layer up).  `scheduler/record.py` claims "stdlib-only ... ships to the target" (`:23-28`) and `diagnostics.local_facts` is placed where it is BECAUSE of that claim (`:482-486`) -- but the shipping list is `("mb_monitor.py", "config_dir.py")`, and `record.py` imports `..persist`, `..config_dir`, `.quantities`, `.admit`.  A constraint two modules reason from that nothing exercises. | drop the claim or make it true; decision |
| **Y4** | `known_machines()` composes user-facing summary strings (`?? not understood`) inside the record module (`record.py:833-962`). | note |
| **Y5** | ✅ closed with J1 (2026-09-13).  J1 (`_dispatch.run_in_env`), sharpened: it also addresses the env BY NAME (`conda_run_argv`, `:73`) while the door re-addresses at the prefix (M2). | J1 |

#### K-D. Redundancy -- two homes for one thing

| | finding |
|---|---|
| **D1** | ✅ **CLOSED 2026-09-13.**  The wizard writes through `write_config_scope`; `emit_molbuilder_json`, `generate_session_secret` and `cli._read_config_object` are deleted; the clobber guard reads through `read_config()` (a file the server would refuse stops the wizard before it asks for a secret, and there is no `--force` past that); the door now says when the EXISTING file was already one the server would refuse, naming the section, instead of blaming the patch -- test with a bad `envs` entry and a good patch, mutant (diagnosis off) fails it; `--force` replaces the providers list and keeps `auth.trust_proxy` (tested).  `configuration.md` § 2.3 loses the wizard's row and paragraph; § 2.1e names the real resolver and creator.  J2, sharpened.  `read_config(path)` is `json.loads` -> `_normalise` -> prefix the path (`runtime_config.py:109-151`), so `emit_molbuilder_json`'s "validate the bytes on disk" is the same validation `write_config_scope` runs in memory (`:2233`).  The wizard's merge REPLACES `auth` wholesale, so re-running it to add a provider drops `auth.trust_proxy`; the door's deep-merge keeps it.  The door's one gap -- it blames the patch when the existing file was already invalid (`:2229-2236`) -- is exactly what the wizard solved privately.  `cli._read_config_object` (`:1443`) is a third reader of the format, and `auth-setup` reads the same file three times in one run (`:1609`, `:1720`, `auth_setup.py:404`). |
| **D2** | ✅ **CLOSED 2026-09-13.**  `config_dir.PRIVATE_DIR_MODE` / `PRIVATE_FILE_MODE`, the other two pairs deleted (`CREDENTIAL_FILE_MODE` renamed: a run-report record is 0600 and is not a credential).  Three pairs of mode constants for two values: `CONFIG_FILE_MODE`/`CONFIG_DIR_MODE` (`runtime_config.py:1266`), `CREDENTIAL_FILE_MODE`/`PRIVATE_DIR_MODE` (`config_dir.py:66`), `REPORT_MODE`/`REPORT_DIR_MODE` (`notify.py:172`).  The writer of `molbuilder.json` uses one pair; the audit checks the same file with another. |
| **D3** | ✅ **CLOSED 2026-09-13.**  Callers ask `config_dir` directly; the pass-throughs and `_mkdir_private` are deleted, their reasoning moved to the two call sites (`supervise`, `LogRoll`).  `serve_daemon.run_dir` / `pid_path` / `log_path` / `stacks_path` are four one-line pass-throughs to `config_dir` doors (`:42-80`) -- I8's shape; `_mkdir_private` (`:87`) is a third name for `ensure_private_dir(tighten=True)`. |
| **D4** | ✅ **CLOSED 2026-09-13.**  `_read_server_wide`, `_read_scope`, `_read_section` deleted (callers say `read_config()` / `_require_object_section(...) or {}`); the getters take the section `read_config` already validated instead of validating it again; `get_paths`'s re-raise and `get_scheduler`'s two unreachable branches gone.  Inside `runtime_config`: `_read_server_wide` is `read_config()` (`:1513`); `_read_scope` is `read_config(path)` with a pre-check the callee already makes (`:1189`); `_read_section` and `_require_object_section` both do "section as dict or raise" (`:154`, `:549`); `get_paths` re-validates an already-validated section inside a `try/except` that re-raises (`:1946`); `get_checkpoint*` and `get_script_generation` re-run validators on normalised output (`:1023`, `:1643`); `get_scheduler`'s two `elif ... _validate_scheduler(raw)` branches are unreachable (`:2010-2016`) -- the same dead guard removed from `get_execution` the day before. |
| **D5** | ✅ **CLOSED 2026-09-13.**  Returns the `Path`; the `via` is the literal `"config-dir"` at the two displays that print it.  `machine_config_path()` returns `(path, "config-dir")` and the second element has been a constant since the cwd step was deleted; 14 callers index `[0]`. |
| **D6** | ✅ **CLOSED 2026-09-13** for the readers: `diagnostics.manager_info` is the one reader of `info --json` (prefix resolver, state probe, `init-config`), and `install._prefix_under_envs_dirs` the one `envs_dirs` walk.  The capture wrappers are left: `validate._run` (never raises, combined output), `builds._run_capture` (a `CompletedProcess`) and `record._run` (stdlib, output-or-None) answer three different questions.  J3: `info --json` has three readers and the `envs_dirs` orphan walk is spelled twice in `install.py`.  Six private "run a command and capture" wrappers across `builds`, `validate`, `abi`, `record`, `diagnostics`, `initconfig`, each slightly different. |
| **D7** | *Reduced 2026-09-13* by L1/L3: the install verb reads `state.prefix`; what remains is `run_install`'s one resolution plus `ensure_prefix` when that gave none, and one per other verb.  The env prefix is resolved up to four times per install: `probe_env_state` puts it in `state.prefix`, then `_cli.py:1395`, `:1457` and `run_install:1257` each ask `_env_prefix` again. |
...
| **D10** | ✅ **CLOSED 2026-09-13.**  `builds.tree_bytes` is the one walk; `hints.stdin_can_answer` is the one copy, below both callers; `cmd_bootstrap` calls `initialize()`; `fix_cmd(action, *args)` spells the verbs with no recipe too.  `cmd_clean._du` vs `builds.env_size_reference_gb`; `_stdin_can_answer` copied from `cli._stdin_is_a_terminal` "because A7" (`_cli.py:2057`) -- the one function should move down, not be copied; `cmd_bootstrap` spells `initialize()` as `detect()` + `set_capabilities()` (`:1911`); `hints.fix_cmd` needs three call-site workarounds for a verb with no recipe (`_cli.py:250`, `:1339`, `:1896`). |
| **D11** | ✅ **CLOSED 2026-09-13** for the two production shapes: `read_notify_keys()` and `issue_notify_key()` take no path, and `notify-token` resolves nothing to pass.  `load_channels(path=)` stays: three tests point it at a file of their own.  `read_notify_keys(path=)`, `issue_notify_key(path=)`, `load_channels(path=)`: `cli.py:1992-1998` resolves `notify_keys_path()` and passes it to a function that would resolve the same default -- the resolved-then-passed shape removed for `MB_NOTIFY_KEYS_FILE` the day before. |
...
| **X5** | ✅ **CLOSED 2026-09-13.**  Defined at the top of the function.  `cmd_bootstrap:1955` tests `'failures' in dir()` for a local defined in one branch. |

#### K-S. Statements that make someone do the wrong thing

| | where | says | truth |
|---|---|---|---|
| **S1** ✅ closed 2026-09-13 | `config_dir.py:130-135` | `paths.logs` is applied by `runtime_config.logs_dir` | the key is refused; that function does not exist |
| **S2** ✅ closed 2026-09-13 | `runtime_config.py:30-32` | flat `cert`/`key` are "honoured (folded into tls)" | refused by name (`:772-776`) |
| **S3** ✅ closed 2026-09-13 | `install-env.sh:93-94` | `./molbuilder.json` overrides "are read from CWD" | retired 2026-08-31 |
| **S4** ✅ closed 2026-09-13 (the help now names both differences) | `install-env.sh:255-272` | entry points (a) and (b) are equivalent except `--gcc` | (b) has no disk probe for the manager (`_find_conda_binary` vs `detect_env_mgr` step 4): a shell where conda is only a function finds a manager through the shim and none through `python -m molbuilder` |
...
| **S6** ✅ closed 2026-09-13 | `runtime_config.py:1336-1338` | the config is written by an `fchmod`-before-first-byte writer | that writer is gone; `write_bytes(mode=)` is |
| **S7** ✅ closed with L1 (the `_Dispatcher` docstring was rewritten there) | `install.py:981`, `:1252-1256` | resolving a prefix costs "3-5 registry calls" | usually zero since the snapshot strategy |

### R. Fresh-eyes review of the pushed tree *(2026-09-14, four agents, every claim verified by hand before it was acted on)*

*(Reports: `review-A-diff.md` (the 33 pushed commits), `review-B-envs.md` (the installer set), `review-C-config.md` (the config set), `review-D-tests.md` (the tests). Below, only what held up. "Measured" = reproduced on this machine (conda 26.7.1) or with a script in a temp directory.)*

| | finding (source) | status |
|---|---|---|
| **R1** | **`--clean` on a fresh machine died at step 1**: `env remove -n` exits 1 for an env that is not there (measured), and the removal was dispatched unconditionally (B-L1). | ✅ closed: the runner SKIPS the removal when nothing is on disk; test + mutant |
| **R2** | **K-L9's premise was false**: `env remove --prefix` is refused for a directory without `conda-meta/history` (measured), which is exactly what an ORPHAN under `envs_dirs` is; the fallback could never be taken (B-L1). | ✅ closed: fallback removed; ORPHAN/BROKEN refuse `--clean` with `rm -rf <dir>` named; the false-premise test retired |
| **R3** | **`-n` addressing fails for a healthy env outside `envs_dirs`** -- conda's `locate_prefix_by_name` searches `envs_dirs` only (read in conda's source) -- so `repair`'s `conda install -n` and `--clean`'s `env remove -n` could not reach the case the contract calls supported (B-L2, A-3.1 sharpened). | ✅ closed: the door re-addresses `install` and `env remove` at the prefix like `run`; `repair` hands its prefix over; `remove_env_cmd` spells `--prefix` when a directory is known; test + mutant |
| **R4** | **`addressed_by_prefix` scanned the whole argv for `-n`** and rewrote an inner command's own `-n` (`mpirun -n 2`) inside a prefix-addressed `run` (A-1.2, measured). | ✅ closed: positional; test + mutant |
| **R5** | `--clean` asked for confirmation only when the registry listed the env, so an unlisted directory would have gone unasked once R2's fallback worked (B-L3, A-1.1). | ✅ closed: asks whenever a directory exists |
| **R6** | `--clean --force-resume` skips the create the wipe needs (B-L9); `bootstrap` with no manager died with a `TypeError` (B-L4). | ✅ closed: refused / guarded |
| **R7** | The "manager's `run` is unusable" note was printed only to a sink, so every quiet dispatch (`doctor`, the presence gate, a tool call) switched silently against M4 (A-1.3). | ✅ closed: to stderr, once; asserted in the fallback test |
| **R8** | `doctor`'s verify created `var/tmp` in every env it audited (B-Y2); the preflight's disk reference walked every directory beside the env -- all of scratch for a `--prefix` env (B-L6); host CUDA was reported though the build strips it (B-L8); the CUDA/gcc remedy contradicted its own table (B-L7); `info --json` was read in conda's key names only (B-L5, unverified on mamba 2 -- both dialects read now). | ✅ closed |
| **R9** | Three surfaces parsed `-optional` off the issue's `kind` string (B-R1); the wipe was inserted into the plan at two sites (A-3.1); dead `validate_recipe(quiet=)`, a `"wipe"` event nothing emits, an `init-config` spelling workaround (B-R4/R5); `caps.conda_envs` membership hand-rolled beside `env_available` and three literal `molbuilder envs install` spellings (B-Y3). | ✅ closed: `optional` is a field; `run_install` takes its plan from `plan_install`; deleted / through the doors |
| **R10** | Config: the one creator left every missing ANCESTOR at the umask (root 0755 measured); `init-config` was a second writer of `molbuilder.json`; the writer checked scope on the patch only; refusals named the wrong file (three shapes); the shadow warning fired on the config directory itself; the mode warning misread a 0700 file (C-L1..L7, C-Y1). | ✅ closed in `e8a49a2f` with tests + mutants |
| **R11** | Sentences that would mislead: `run_step`'s deleted `env` parameter still named as the way (env-framework § 5.6); `auth_setup` named as the writer of `molbuilder.json` (configuration § 2.1b); "sentinels + a toolchain fingerprint" (installation § 6); the flat `cert`/`key` "accepted" (`cli._resolve_tls`); two closed defects still marked open (H1, H10); `{cuda_cc}` and "system CUDA root" in the recipe docstring; "default 14". | ✅ closed |
| **R12** | **Decisions taken by the owner 2026-09-14.**  (a) *"clean up old names, we need explicit consistent setup"* -- `preactivate`, `preactivate_format` and `autodetect_conda` are REFUSED by name, each with its own sentence; the alias and the warn-and-drop are gone, `running-a-job.md` § 5.2 says so, and one test pins the refusal in place of two that pinned the tolerance.  (b) *"we are explicit and strict about security related files ... handled by unified api, no arbitrary place"* -- `load_channels(path=)` deleted (for Slack and Discord the URL IS the credential); ten test call sites point `MOLBUILDER_CONFIG_DIR` instead.  **The project scope STAYS**: I recommended deleting it on the evidence that nothing writes one, and that was backwards -- `configuration.md` puts `.molbuilder.json` in the *written by a person* row ("nothing in the program may overwrite it"), it was deliberately wired into submission on 2026-08-12 (R3), and deleting it would re-open the gap that retiring the cwd `molbuilder.json` closed.  `task.json` cannot take its place: `validation/task.py` refuses machine facts there by name.  (c) *"ELPA does not require cuda -- that was a misconception"* -- see R14.  (d) `repair`'s policy in the surface (B-Y1) remains a refactor nobody has asked for. | ✅ closed |
| **R14** | **The GPU recipe said things about CUDA that are not true** (user, 2026-09-14): ELPA is a dense-eigensolver library whose CPU kernels stand on their own -- it is built with `--enable-nvidia-gpu` so the GPU path EXISTS, not because it needs CUDA; the toolkit comes from conda INTO the env and the host's CUDA is never read; no GPU and no driver are needed to BUILD; the driver is a run-time concern on the machine that runs the job, and belongs in `system_preconditions` as a note. | ✅ closed: `cuda_min_version` deleted (field, preflight branch, and the 12.4 -- never ELPA's requirement; ELPA 2024.05.001 documents no minimum and its changelog's only CUDA number is a workaround FOR < 12.1; it was a fossil of an early gcc-pairing belief, and it gated a version this recipe pins itself).  `BuildComponent.needs_cuda` deleted with the docstring that claimed it.  `cuda_required`, the driver paragraph in `builds.py` and `installation.md` § 6 now state the model outright, including that the gcc 14.3 pin is SIESTA 5.4.2's `kpoint_t.F90` miscompile and NOT CUDA.  `system_preconditions` was accurate and rendered nowhere: `install --dry-run` prints it, and a test asserts the driver line says OPTIONAL. |
| **R13** | Noted, not acted on: `_env_prefix` reads the process snapshot while its callers pass a snapshot down (A-3.2); `machine_config_mode_warning` and placement's row phrase one fact twice, by design (C-R6). | note |
