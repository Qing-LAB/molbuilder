...
| **H13** | **T14** | **`<mgr> run -n <name>` cannot address an env that lives outside `envs_dirs`** — conda's own `locate_prefix_by_name` (`conda/base/context.py:2239-2258`) searches `envs_dirs` and raises `EnvironmentNameNotFound` otherwise. So the M1 primary form has to address an env by the **prefix the registry gave us** (`run --prefix <prefix>`), which is what `builds.py` already does while `install.py` uses `-n`; the two addressing forms are a second thing `_bypass_conda_run` has to special-case (`argv[2] in ("-n", "--prefix")`). Found while fixing H1, which makes such an env reachable for the first time. **Phase 1, with M1** | **READ** (conda source) |
...
**1a. The dispatch stops imitating the manager** *(pulled forward from Phase 4,
2026-09-12, on the user's ruling)*. `conda_run_argv` becomes the primary form and
...
**1b. Then the config-placement half.** **Correct the rule before the sites**,
because A11 as written today names a pre-consolidation owner and so licenses the
climbs.
