...
| # | item | from | state |
|---|---|---|---|
| **N1** | **RULE SETTLED 2026-09-03 — *anything over three seconds runs as a job, show progress* (`web-api.md` § 1a). The MECHANISM is not built.** Measuring first changed what the item is: the two examples behave oppositely. An RDKit embed of a C₆₀ alkane takes **4.5 s and costs one thread** — it releases the GIL, others run at 99%. A 25 MB SIESTA `.out` parse takes **4.7 s and drops every other request to ~8%** — pure Python, holds the GIL (51 MB: 9.6 s). So the slow thing and the freezing thing are different things, and the rule has two halves: over 3 s leaves the request whatever it costs others; pure-Python work leaves it even under 3 s, into a SUBPROCESS (a thread holds the same lock) — the shape the GPU half already took. **The live instance**: `_refresh_if_changed` re-parses the WHOLE file whenever mtime advanced, and a running job's mtime advances constantly, so every watching viewer pays it every poll. Its docstring said dropping the lock kept others unblocked; that is corrected — releasing the lock lets another request ENTER, not RUN | audit 08-28 O4 | rule done · mechanism open |
