...
> **A narrative handover for these four rows** — what the work was for, what changed and why, and what to do first — is [`plans/2026-09-22-handover.md`](?doc=plans/2026-09-22-handover.md). Read it before acting on **X1**, **X2**, **X3**, **X4** or **D5**: the rows below are evidence, the handover is the context.

> ## ⛔ DECIDE THESE FIRST — two rows are blocked until someone does
>
> Both are **design decisions, not implementation choices**, and both have
> already cost work: each was answered implicitly by code, the implicit answer
> was wrong, and the work was reverted or lost. **They belong at the start of
> a session, not in the middle of one.**
>
> | # | the question | what it blocks | where it is argued |
> |---|---|---|---|
> | **D-1** | **What declares that a value binds EVERY rung?** `citation` says *who supplies the default*; the shared class is larger — `species_order`, `spin_treatment`, `spin_total` and the pseudopotentials bind all five rungs and no run answers them. Sibling marker `shared = [...]`, or widen `citation`? | **W30 ① and therefore ②–⑤** — the whole transport parameter surface. Also closes a hole in `prep` that predates it | `engines/transport.md` § 3.8.6 |
> | **D-2** | **Where is a mode's activity CLASSIFIED, and by what rule?** `spectra.json` carries no classification and its zeros are numerical (`4.8e-09`), so *"is this IR-active"* needs ONE stored home — never an epsilon invented in the viewer. And the per-mode ES probe selects by **Raman** brightness, which in a centrosymmetric molecule drops every IR-active mode | **W21 entirely** — the mirror plot, the rug and the threshold all read the answer | `plans/plan.md` W21, prerequisites (a) and (b) |
>
> Narrative for both: [`plans/2026-09-23-handover.md`](?doc=plans/2026-09-23-handover.md).
...
| **V1** | engine / science | **Vibrations with held atoms — one path, not two.** A live BDT run on 2026-09-21 (free vs. both sulfurs held, one geometry, driven through the web UI) measured a **36th "mode" that is the ring turning about the S···S line** — 100.0 % of that motion, at −0.93 cm⁻¹, not a vibration. Three places in the code claim to know how many such non-vibrations exist and **all three disagree with each other and with the measurement** (deck: *"all 3·N_FREE eigenvalues are physical"*; `spectra/methods.py`: 30; `validation/spectra.py`: *"2-ish"*; truth: **1**). Root cause is the shape, not the arithmetic: the held-atom case is a **branch beside** the free case, and the two have drifted in four independent ways — masses (fixed 2026-09-21; had been 1823× on infrared intensities), whole-body-motion removal, the mode count in the Methods paragraph, and the stationarity check (reads held atoms' forces, which are non-zero by construction). Two further measured items: the thermochemistry excludes the spurious mode **only because noise put it at −0.93 rather than +0.93 cm⁻¹** (≈12.7 cal/mol/K, ~3.8 kcal/mol in G), and **freezing does not cut the cost** the docs promise — the full Hessian is computed and sliced, though PySCF's `atmlst` would deliver the saving. Science contract: [`science/normal-modes.md`](?doc=science/normal-modes.md) (rules R1–R8, verified against Crossref + 8 sources, full texts in `science/refs/`). **The follow-up list is [`2026-09-22-vibration-audit.md`](?doc=plans/2026-09-22-vibration-audit.md)** — every remaining item with its evidence, in plain language. Design and options: [`2026-09-21-normal-mode-unification-design.md`](?doc=plans/2026-09-21-normal-mode-unification-design.md) **SIESTA half added 2026-09-21** *(user: "siesta does not need to provide Raman/IR intensity … but it can be helpful for connecting vibration with transportation")*. The Spectrum tab offers **one engine** (`choices: ("pyscf",)`) and SIESTA's force-constant machinery appears **nowhere** in the repo — though `vibra` is already installed in `molbuilder-siesta/bin/`, and the project's own layout already splits `frequency/` (*"Hessian + harmonic thermochemistry"*) from `spectrum/` (*"Raman, IR"*), which is exactly the split asked for. For a junction the PySCF route is the wrong one: it forces an Au(111) slab into a fake finite cluster and puts the vibration on a **different potential-energy surface** from the transport it feeds. **The contract gained a generalisation** (`normal-modes.md` § 3.1a, verified): a periodic system has **3** whole-body zero modes, not 6 — rotation is not a symmetry when the box does not turn — and **0** once any atom is held, so the spurious-mode problem is *molecular* and vanishes for a held slab. **The one hard constraint:** SIESTA's `FC.First`/`FC.Last` is a **contiguous atom range** (read from our own binary, which also carries `FC.Save.dHS` — the electron-phonon ingredients for IETS), while our held set is scattered; so the free atoms must be renumbered, which belongs in `engine_atom_index.py` and crosses the transport path's electrode-by-index assumption. IR/Raman are to be **not drawn** for SIESTA rather than drawn-and-ignored (`molview/ui.js`: *"a control that silently does nothing is a bad answer for a user"*). `Frederiksen2007` + `Galperin2007` were already in the bibliography, uncited — the vibration→transport direction was anticipated **State of practice checked 2026-09-21** and it changes the risk, not the direction: **PySCF has no frozen vibrational analysis at all** (none of the 8 entry points in `pyscf.hessian.thermo` takes an atom list; nothing in `pyscf-forge`/`pyscf/properties`; issue tracker clean), and **ASE — the most-used implementation — does line-for-line what our deck does** (`Vibrations(indices=…)` → mass-weight → `np.linalg.eigh`, **no projection**, 3n modes) [ASE2017]. So the hand-written frozen path is the *community standard*, the leftover-motion wart is community-wide, and there is **no upstream implementation to borrow or to second-guess**. The real risk is the opposite one: **nobody computes how many motions survive from the held set's geometry** — ASE projects none, an ASE-based PR projects all six (wrong when atoms are held), Vester & Olsen remove by character after the fact — so the rank rule is ahead of the field and nobody else's testing covers it. **Two traps found that a five-row table gets wrong and the rank gets right: CO₂ with both O held and acetylene with both C held are `n_rigid = 0`, not 1** (the free atoms lie *on* the axis). Contract updated accordingly; `Ghysels2010` + `ASE2017` added and verified | BDT e2e 2026-09-21 · SIESTA + practice survey 2026-09-21 | **Option A DECIDED (user, 2026-09-21)** — one path, with free-case equivalence as a build gate. Order of work + test systems in the design § 7.5/§ 7.6. Cost/IR fork DECIDED (a run-setup option). **Open:** the SIESTA contiguity answer, the cost/IR default, sequencing |
...
| **W20** | front end | **A prep from the browser can never produce a pipeline log.** `pipeline_log` is off by default and only `jobset prep --pipeline-log` sets it (`_cli.py:2252`; re-measured 2026-09-23); the web door builds its kwargs at `build.py:1639/1663/1682` and calls `prep_calculation` at `:1685` without it (re-measured 2026-09-23). Measured: 2 `*.pipeline.log` in the tree, both from e2e fixtures, **none** under any real project. Not a defect — the flag is deliberately opt-in (`pipeline_log.py`: *"the log observes the pipeline, it is not a step in it"*) — but if a person prepping from the UI should be able to ask for one, the door needs a way to say so | found 2026-09-11 | **needs your call** |
