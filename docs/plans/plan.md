...
| **TS8** | **The science redesigns — two closed 2026-09-09, and the USER'S CORRECTION is the durable lesson.** My first pass made both assertions numerically exact about **ASE's** crystallography — the extent's linearity in `a`, the ABAB step — when `add_slab` computes no distance at all: it asks ASE for `n_layers + period - 1` layers and keeps `zs[first:first+n_layers]`. **THE CUT IS THE ONLY THING THIS LAYER OWNS.** Both now source their expectation FROM ASE, and the mechanism is covered by a new structural, element-agnostic test over four metals × three faces × three registries × four layer counts (144 cases) asserting the slab IS that window — shapes AND the registry-sensitive displacement sequence, no distance anywhere. An off-by-one registry kills 108 of 144; it survived the shape check alone, because `_shapes` is deliberately blind to registry. **Remaining: §§ 3–7a**, sharpest is the `homo_idx` gap | each fails for the reason it names, and asserts nothing we did not write |
