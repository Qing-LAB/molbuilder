...
| **TS7** | **THE HARNESS WAS BROKEN TWICE, AND THE AUDIT'S ERROR RATE IS ~50%, not 1 in 5.** (a) It loaded its coverage plug-in as `-p covplug`, never committed — the repo's is `tools/_pertest_coverage.py` — and failed SILENTLY, reporting `NO-COVERAGE` for everything. (b) It had **no operator that drops a clause from an `or`**, which is the commonest defect shape in a validator: a guard is `not isinstance(x, T) or not x` and its two tests reach ONE CLAUSE EACH, so every other operator kills both together. Both fixed. Re-run of 22 pairs / **19 candidates: 9 must be KEPT, 10 cuttable** — the three it had cleared for deletion include `providers: []` and `id: ""`, whose loss is a server that starts with a login page and no way through. **Remaining: apply the 10, and the 4 subprocess pairs need a different method** (`sys.settrace` does not follow a child) | every applied cut has a mutant from the FIXED operator set; NO-COVERAGE is never read as a verdict |
...
| **TS13** | **The rest of `test_auth_config.py` — ~75 tests still `_normalise(dict)`, one malformed value each.** `TS11` did the providers group and showed the shape; this is the same treatment for the per-entry, Google, GitHub, Microsoft, ORCID, CAS and trust-proxy groups. **This is where the count actually falls**: TS11 went 83→80 functions but 128→131 collected, because seven rows replaced five functions. The reduction comes from the ~75 | one table per config section, rows being (broken file, refusal it must give); the count drops and every case is readable |
| **TS14** | **Two rules still written twice, both recorded not fixed.** The depth-2 canonical-topic rule lives inline in `files.py::api_files_rename` (`:1781`) and `::api_files_delete` (`:2359`) — `#69` deleted the dead third copy but merging the live two is a design change. And the Makov-Payne formula is in `makov_payne.py:126` (real) and `:327` (inside the shipped script's text); measured to agree today, and the shipped script is stdlib-only by choice, not necessity | each rule has one home, or a note saying why two is deliberate |
