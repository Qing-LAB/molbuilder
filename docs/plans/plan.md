...
| **E9** | **C8 CLOSED 2026-09-03 — the third state does not exist any more, and its residue is deleted.** The design was revised on 2026-08-15 from three states to TWO: unset omits the keyword (SIESTA's own automatic), a value is honoured verbatim, and `0` — the old *"omit"* sentinel — is REFUSED by the validator. Two survivors of the old shape were still in `siesta/input.py`: a comment paragraph asserting the three states four lines above the one asserting two, and a `block_size == 0` arm in PROVENANCE printing *"omitted (SIESTA's own)"* — which would have **lied**, because with 0 the emitter writes `BlockSize 0` into the deck (0 is not None). **C12 (the rank-clamp message) still needs a call from you** about what it should say | roadmap § 6 | C8 done · C12 needs a call |
