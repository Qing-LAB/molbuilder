...
| **W30** | front end | **A restore and a re-read of the same file read identically on screen**, and they have different consequences — one brings back work you had in progress, the other throws it away for what is on disk. Saying which happened is one line of status text. | `archive/2026-09-21-modify-persistency-investigation.md` § 5 (archived 2026-09-21) | **OPEN — a nicety, not a defect.** Its own document called it *"worth a sentence if it is wanted"*; carried here only so archiving that record does not silently drop it |
