...
| **TS8** | **The science redesigns — STARTED 2026-09-09 with a filter the Makov-Payne detour produced: *which number does a person actually use, and does the test point at it?* Two closed under it.** `test_electrode_lattice_constant_override` asserted `e_extent > d_extent + 0.5`, satisfied by ANY lattice constant above the default; the ratio is exactly `5.0/a` and is now asserted at `rel=1e-9` (a 2% error passed before, fails now). `test_the_same_holds_on_the_two_period_surfaces` asserted `moved > 0.1` — *"the registry did nothing"* — where its (111) sibling has always compared against `a/√6`; (100) is `a/2` and **(110) is `a√6/4`**, which corrects this record's own derivation. **Remaining: §§ 3–7a**, and the sharpest is the `homo_idx` gap — nothing checks it is the index of the highest OCCUPIED orbital | each fails for the physical reason it names |
