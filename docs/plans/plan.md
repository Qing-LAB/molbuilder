...
| **W27** | execution / engines / science | **A junction walked end to end, 2026-09-15 — 19 findings, record in [`execution/walkthrough-2026-09-15-junction.md`](?doc=execution/walkthrough-2026-09-15-junction.md).** *(user: "build a test case … simulate the whole calculation … do not hack and be honest to detect the missing gaps".)*  A 37-atom Au(100)/S-CH2-S/Au(100) junction, built + labelled + relaxed (concluded in 41 s) + cited + described, in BOTH directory shapes, through the real doors.  **Verified working:** the whole front half — labels through the Filter/Assign door, the hand-over, Task setup's no-default shape ask, `prep`'s printout, a correct deck in every particular, immutable attempts, and `fdf`-is-truth on the citation.  **THE TWO BLOCKERS.** ① **F17** `validation/task.py::config_class_for` dispatches on `task.engine` only, so a transport description (engine `siesta`, correctly — TranSIESTA has no separate binary) is validated against `SiestaConfig` and every transmission/contour field is refused: `jobset prep run seed` cannot prep what the Transport tab writes, while the web prep door — which does not run that validator — preps it fine.  `task.calculation == "transport"` is in the file and never consulted.  ② **F18** that web prep then writes `01_seed/` with **no `run-N`**, so `launch` refuses with a remedy pointing back at the blocked CLI prep (measured NOT general: the same endpoint opens `run-2` for the SIESTA ladder).  **Net: the five-stage ladder cannot be launched by any door**, so `electrode_L`, `electrode_R`, `device`, `transmission` and `summarize run` were never reached.  This is what W26's "the Task setup seam works" missed — it was verified against `prep-plan`, which only PLANS.  **THE WORST SILENT ONE. ③ F9** — `--electrode` passes `sequence` but leaves `start_registry` at 0 on both slabs (no spec field for it), so EVERY CLI-built junction has a defective seam: measured `eclipsed` on (100)/(110) and **twin** on (111), for all six layer counts `junction-cell.md` § 3.1 says continue the crystal.  `start_registry` is the control and works (+z=1/−z=0 → continues, gap = a/√2).  **F10**: § 3.2 states the wrong mechanism for the CLI, which is why this went unnoticed.  **④ F13** every `%block`-valued parameter is printed in the run log under "not in the deck; the engine default applies" — including a k-grid that IS set — because the read-back cannot parse a block; load-bearing, since the composite reads the transverse k off that deck.  **⑤ F6** `classify_seam` is right (measured `collision` at 0.00 Å) and has ONE consumer, the web slab endpoint; the CLI build, the cell-apply path and `validate` all miss it, and `validate` calls a 0.00 Å image collision "suspiciously tight".  **⑥ F15** `transport.md` § 4 calls `bridge` implicit; the composer requires it explicitly, and refuses the citation after the relaxation has run.  F3/F4/F5 (CLI `--orthogonal` help inverted, a promised validator that does not exist, slab default `a_experimental` where a PBE run wants `a_pbe`), F11/F12/F14/F16/F19 in the record | walkthrough doc · found 2026-09-15 | ① ② ③ ④ ⑤ ⑥ **open** · F4/F9/F13/F17 pinned xfail in `tests/test_workflow_junction_walkthrough.py` |
