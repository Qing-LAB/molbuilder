...
| **TS15** | **`#80` — the X3DNA probe answers *installed* for a pack that cannot run** (`bin/x3dna_utils` is a ruby script; ruby is absent), so six DNA tests FAIL with `exit 127` where they should SKIP. Same question as `TS10`, opposite symptom: node's absence is honest, this one is not. **Needs a decision**: check runnability rather than presence, or install the interpreter | the probe separates *present* from *usable*, and the six tests skip or pass |
...
| **TS8** | **The science redesigns — three closed 2026-09-09 under one question: WHO OWNS THE NUMBER?** Two slab tolerances were re-aimed after the user's correction (they asserted ASE's crystallography; `add_slab` only cuts a window, so both now take their expectation from ASE and the mechanism is covered structurally over 144 element/face/registry/count cases). The `homo_idx` gap went the other way: the record asked the READ side, which carries the index but not the occupations it came from — so it cannot be checked there. It is the EMITTER's, it is ours, and it had a branch (restricted vs unrestricted `mo_occ`) living only in generated script text. Now `homo_index`, spliced from its own source, nine cases, five mutants killed. **Remaining: §§ 3–6 and the rest of 7a** | each fails for the reason it names, and asserts nothing we did not write |
