...
| **TS7** | **STARTED 2026-09-09; the harness had to be repaired first, and the number got WORSE.** The committed tool loaded its coverage plug-in as `-p covplug` — never committed — while the repo's is `tools/_pertest_coverage.py`, and it failed SILENTLY (empty coverage reads as `NO-COVERAGE`, which looks like a finding); it also required `CLAUDE_JOB_DIR` and so ran only in the session that wrote it. Both fixed. Then 8 of § 5's verdicts went through it: **2 NOT-SUBSUMED (do not cut), 2 CONFIRMED, 1 inconclusive, 4 undecidable by REACH** (subprocess probes — `sys.settrace` does not follow a child). **Running total 3 wrong in 9 decided — one in three, not one in five.** Both new false verdicts are the same shape: the candidate drives a ROUTE that reaches an arm of the fence the named coverer's input never reaches. **Remaining: the 22 header-pass candidates, and the 4 subprocess pairs need a different method** | every applied cut has a mutant; NO-COVERAGE is never read as a verdict |
