...
| **W27** | engines / execution | **Transport never adopted the template framework — rebuild the parameter surface on it.** *(user: "you are not following this design of using a template data driven model and the framework of script generating workflow/layout"; "if transport task is unique enough … i don't mind to have a separate template and document it as so. but you never investigated or justified if this is the case".)*  The design, the justification and the outcome are `engines/transport.md` §§ 3.2–3.6 — **measured**: a transport template inherits 49 siesta items of which only 9 are the relaxation driver, because every transport stage IS a SIESTA run, so transport is the siesta base minus that driver plus the NEGF/TBtrans rows, tagged `calculations = ["transport"]` in the one catalogue.  **What is wrong today:** parameters DEFINED in `TransportConfig` (32 fields, 0 catalogue rows) against `template.md` § 2.1; `dataclass_to_form_schema`'s last caller; `_prep_transport` bypassing `resolve`, `spec_for` and `prepare_deck`, so no stage deck has a `.validation.txt` or a USER-CUSTOM zone; a private `_emit_header`; `electrode_kz` (invariant I9) a Python function default no description can reach; `max_memory_mb`/`num_threads` machine facts on floor 2 reaching the deck only as comments; the five-bag override merge making per-stage variation impossible against archive § 4.2.  **The one extension asked for** is a sibling of `allocation` marking an item the CITATION answers (§ 3.3.3) — it replaces `SEALED_ALWAYS`, `CONTRACT_FIELDS` and a predicate spelled twice.  **Shipped defects that dissolve with it rather than being patched:** `TBT.k` emitted as a bare scalar the parser cannot read (dead, and the flagship of c64d567e); `tbt_k_grid`'s transport axis unguarded; the binary guard vacuous for 1-char stems and blind to value syntax; `part line` satisfied by the emitter's own comment.  *(W27 previously recorded two blockers, F17 and F18, as CRITICAL.  Both were wrong: with `varies` empty the CLI preps and launches transport fine, and prepping from the browser was never a door — `task-setup.md` § 1 and `project-layout.md` § 2.1 put prep and launch in the terminal.  The walkthrough document that carried them is withdrawn; its one measured finding survives as `tests/test_electrode_pair_seam.py`.)* | `engines/transport.md` §§ 3.2–3.6 · 2026-09-15 | design **settled**; implementation open |
