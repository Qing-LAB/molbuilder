...
| **E** ◐ | **T3 per page**, one page per commit: tokens → namespace → residue. `modify`'s **values** pass and are the worked example — but its **composition** does not, measured in a browser 2026-08-30: three card titles on the page at three different left edges, six left edges inside one card, and two typefaces (`plans/modify-redesign-plan.md` § 5). **A token pass is not a layout pass**, and this row said "done" for four weeks on the strength of one. `modify` finishes under that plan's item 5, which also renames `.card-header` and `.ghost` — both written by `modify.html` and by nothing else — out of the shared namespace, retiring two of D's allowlist entries. Then `spectra`, `structure-optimization`, `transport`, `results`, `documents` | independent, and a browser check per page is cheap |
