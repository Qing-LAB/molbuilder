...
| `resolve_environment(overrides=…)` exists and no caller passes it, so a machine fact cannot be declared at all | this document, § 5 M-5 | open by design — the door is described, not built |
