...
| Transport (TranSIESTA) | `/api/transport/render` | NO — TranSIESTA's transport calculation uses `struct.regions` (electrode_1 / channel / electrode_2) for electrode definitions; the engine has no `Geometry.Constraints` emitter at all (verified by grep on `transport/transiesta.py`: zero `frozen_atoms` consumers in `render_script` / `_emit_geometry`). | YES — required; preflight ERRORS if any of the 3 expected regions are missing or empty (`transiesta.py::TransiestaEngine.preflight`, the structural-checks block). | Different boundary-condition primitive than build (regions, not frozen atoms). |
